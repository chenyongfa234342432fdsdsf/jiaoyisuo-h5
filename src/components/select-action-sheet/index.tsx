...
import { ActionSheet, ActionSheetProps } from '@nbit/vant'
          ...
          >
            ...
            >
              {label && (
                <span>
                  <span>{label}</span>
                  {showSelectTradePairValue && <span>: </span>}
                </span>
              )}
              {showSelectTradePairValue && <span>{selectedAction?.nameInLabel || selectedAction?.name}</span>}
            </div>
            {cartIcon || (
              <Icon name="icon_trade_drop" hiddenMarginTop className="text-xs scale-75 translate-y-px" hasTheme />
            )}
          </div>
        </div>
      )}
