...
/**
 * 判断开始时间和结束时间之间是否大于指定时间差
 * @param startTime 开始时间
 * @param endTime 结束时间
 * @param diff 时间差 默认一年
 */
export function CheckTimeDifference(startTime, endTime, diff = 365) {
  const oneYearInMilliseconds = diff * 24 * 60 * 60 * 1000
...
/**
 * 获取指定时间距离时间差的时间戳
 * @param specificDate 指定时间
 * @param isAgo 是否指定时间之前 默认之前
 * @param diff 时间差 默认一年
 */
export function getTimestampFromSpecificDate(specificDate, isAgo = true, diff = 365) {
  const date = new Date(specificDate)
  const oneYearInMilliseconds = diff * 24 * 60 * 60 * 1000
  const timestamp = isAgo ? date.getTime() - oneYearInMilliseconds : date.getTime() + oneYearInMilliseconds
  return timestamp
}
