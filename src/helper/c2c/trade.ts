...
import { getC2cPaymentsManagePageRoutePath, getKycPageRoutePath } from '../route'
...
export function useSpotCoinIdFromFastPay(symbol: string) {
  ...
  const coinRateList = coinRate as ICoinRate[]

  return coinRateList.find((item: ICoinRate) => item.symbol === symbol)?.coinId
}
