...
export function openApp() {
  ...
  // 迭代二，如果打开 app 失败，跳转 app 下载页
  const timer = setTimeout(() => {
    window.location.href = 'https://c6h7n.com/8ZHX'
  }, 5000)
...
/** 判断是否是旧版ios操作系统 */
export const checkIsInBlackList = (): boolean => {
  let userAgent = navigator?.userAgent?.toLowerCase() || ''
  const blackOSList = ['iphone os 13', 'iphone os 12']
  let inBlackOSList = false
  blackOSList.forEach(item => {
    if (userAgent?.indexOf(item) !== -1) {
      inBlackOSList = true
    }
  })
  return inBlackOSList
}
