...
export function getPriceInput({ value, inputValue, max, min, digit, isRound, currentValue }: IGetPriceInputParams) {
  ...
  if (/\.$/.test(newVal)) {
    return digit > 0 ? newVal : newVal.split('.')[0]
  }
  if (newVal.includes('.')) {
  ...
  // 对于最小值大如果限制输入，将无法删除位数，因此最小只能在失焦时填充
  // 对于大于 0 的最小值，始终保证无法输入负数
  if ((typeof min === 'number' && min >= 0 && SafeCalcUtil.sub(newVal, 0).lt(0)) || newVal.includes('-')) {
    newVal = '0'
  }
