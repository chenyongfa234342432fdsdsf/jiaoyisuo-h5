...
export function parseLanguagePageRedirectUrl(pageContext) {
  return pageContext?.urlParsed?.search?.redirect
}

export function getLanguageOnSelectRedirectionUrl(redirectUrl: string) {
  return `/personal-center/settings/language?redirect=${redirectUrl}`
}
