...
import { ICoinRate } from '@/typings/api/assets/assets'
import {
...
import { formatCurrency, formatNumberDecimal, removeDecimalZero } from '../decimal'
...
export const getFuturesPositionList = async () => {
  const assetsFuturesStore = baseAssetsFuturesStore.getState()
  const { isTutorialMode = false } = baseFutureTradeStore.getState()
 ...
 * @returns 仓位保证金
 */
const calculatorPositionOccupationMargin = (data: PositionList) => {
...
export const calculatorCoinRate = (symbol: string, isCalcFloat = true) => {
  ...
  const rate: ICoinRate = coinRateList.find((item: ICoinRate) => item.symbol === symbol) || ({} as ICoinRate)
  ...
  let newRate = 0
  const currencyRate = legalCurrencyRate.find(item => item.currency === currencySymbol?.toUpperCase())?.rate
  if (rate?.symbol && marginCoin?.coinCode) {
    newRate =
      symbol?.toUpperCase() === currencySymbol?.toUpperCase() ? 1 : +SafeCalcUtil.mul(rate.usdtRate, currencyRate)
 ...
 * @returns 合约组总价值
 */
const calculatorGroupAsset = (groupInfo: IPositionGroupList, details: FuturesGroupDetailResp) => {
  let groupAsset = 0

  groupAsset = +SafeCalcUtil.add(
    SafeCalcUtil.add(calculatorMarginCurrencyTotal(false), groupInfo.unrealizedProfitTotal),
 ...
 */
export const calculatorMarginAvailable = (groupInfo: IPositionGroupList, details: FuturesGroupDetailResp) => {
  const { unrealizedProfitTotal, maintMarginRatioTotal, openMarginTotal } = groupInfo || {}
  const groupMargin = SafeCalcUtil.add(
    calculatorMarginCurrencyTotal(true, MarginAssetTypeEnum.availableAmount),
    details?.groupVoucherAmount
  )
...
export const onChangePositionData = (markData: PerpetualIndexPrice[], groupId?: string) => {
  ...
  // 计算合约组总价值/合约组可用保证金总价值，并更新 store
  if (groupId) {
    newGroupList.forEach((groupItem: IPositionGroupList) => {
      if (groupItem.groupId === groupId) {
        const newGroupDetails = {
          ...details,
          groupAsset: `${calculatorGroupAsset(groupItem, details)}`,
          marginAvailable: calculatorMarginAvailable(groupItem, details),
...
/**
 * 计算当前持仓最新价下未实现盈亏、收益、收益率
 * @param positionItem 当前持仓/逐仓详情列表项
 */
export const getLatestPriceProfits = (positionItem: PositionList) => {
  const unrealizedProfit = `${calculatorUnrealizedProfit({
    ...positionItem,
    markPrice: positionItem?.latestPrice,
  })}`
  const profit = `${calculatorProfit({ ...positionItem, unrealizedProfit })}`
  const profitRatio = `${calculatorProfitRatio({ ...positionItem, profit })}`
 ...
 * @param size 持仓数量
 * @param amountOffset 数量精度
 * @param latestPrice 最新价格
 * @param offset 法币精度
 */
export const calculatorPositionSize = (size: string, amountOffset: string, latestPrice: string, offset = 2) => {
  const { tradeUnit } = baseFutureTradeStore.getState().settings || {}

  if (tradeUnit === TradeUnitEnum.indexBase) {
    return formatCurrency(size, +amountOffset, false)
  } else {
