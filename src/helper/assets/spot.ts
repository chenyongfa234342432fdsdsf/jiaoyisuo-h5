...
import { getHybridCoinRate, getQueryCoinPageList, getQuerySubCoinList, getV2CoinRate } from '@/apis/assets/common'
import {
...
export const rateFilter = (params: RateFilterReq) => {
  ...
  if (!amount) return isFormat ? formatCurrency(amount, precision) : amount ?? defaultEmptyText
  ...
  // 获取法币设置的汇率 (兼容获取用户设置的汇率或指定法币汇率)
  const currencyRate = legalCurrencyRate.find(item =>
    rate ? item.currency === rate?.toUpperCase() : item.currency === currencyEnName?.toUpperCase()
  )?.rate
...
export const rateFilterFutures = (params: RateFilterReq) => {
  ...
  const {
    futuresCurrencySettings: { currencySymbol: currencySymbolFutures },
  ...
  if ((symbol || currencySymbolFutures) === (calcSymbol || currencyEnName)) {
    ...
    const currencyRate = legalCurrencyRate.find(item =>
      calcSymbol ? item.currency === calcSymbol?.toUpperCase() : item.currency === currencyEnName?.toUpperCase()
    )?.rate
...
export const rateFilterCoinQuantity = (params: RateFilterReq) => {
  ...
  const rate = legalCurrencyRate.find(item => item.currency === currencySymbol?.toUpperCase())?.rate
  const coinRateInfo = coinRate.find((item: ICoinRate) => item?.symbol === symbol) || ({} as ICoinRate)
 ...
 * @param symbol 提币币种
 * @param fee 提币手续费
 * @param feeSymbol 提币手续费币种
 * @param availableAmount 可用余额
 */
export const calcWithdrawAmount = params => {
  const { amount, symbol, fee, feeSymbol, availableAmount } = params || {}
  let newAmount = '0.00'

  if (!amount || Number(amount) <= 0) {
    newAmount = '0.00'
  } else if (feeSymbol === symbol) {
    const total = `${SafeCalcUtil.add(amount, fee)}`

    if (availableAmount && +total > +availableAmount) {
