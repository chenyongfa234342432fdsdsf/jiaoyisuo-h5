...
import { AssetsCoinListReq } from '@/typings/api/assets/assets'
...
export const onGetAssetsCoinList = async (sort = false) => {
  ...
  const params: AssetsCoinListReq = {
    pageNum: 1,
    pageSize: 0,
    coin: 'USDT',
  }

  if (!isFusionMode) delete params.coin
...
export const onGetContractList = async () => {
  ...
  const res = isTutorialMode ? futuresAccountList : await getAssetsFuturesList({})
...
export const onGetContractOverview = async (isPolling = false) => {
  const { assetsModule } = baseAssetsStore.getState()
  let newFuturesData = { ...assetsModule.futuresAssets }
  ...
  if (newFuturesData?.isOpen) {
    ...
    newFuturesAssets = {
      ...
      totalVoucherAmount: onTotalContractOverview('voucherAmount', newFutures),
    }
  }
