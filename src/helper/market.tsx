...
import { TernaryOptionItemBasic } from '@/typings/api/ternary-option/market'
        ...
        <span className={`cursor-pointer quote-display-wrapper-futures`}>
          <span
            ...
            className={classNames('flex', 'items-center', 'whitespace-nowrap', {
              flex: coinType === KLineChartType.Futures,
            })}
          >
            {forTradeSearch ? (
              <span className="flex flex-col">
                <span className="pair-name-futures">
                  <span className="base-symbol-name quote-symbol-name">{_item.symbolName || _item.symbol}</span>
                </span>
              ...
              <>
                ...
                <span
                  className={classNames('quote-badge', {
                    'ml-2': coinType === KLineChartType.Futures,
                  })}
                >
                  {quoteType}
                </span>
...
export const onTradePairClickTernaryOptionRedirect = (item?: TernaryOptionItemBasic) => {
  if (item && item.symbol) {
    const store = baseTernaryOptionStore.getState()
    const marketListStore = baseMarketListStore.getState().ternaryOptionMarketsTradeModule
    if (item.id === store.currentCoin.id) {
      return
    }
    marketListStore.resetSearchState()
...
export function formatTradePair(item: CommonTradePairDataWithMarketCap | any) {
  ...
  return {
    ...
    base() {
      return <span className="text-text_color_02">{formatTradePair(item).lastByUserPreference()}</span>
    },
    marketCap() {
      return (
        <>
