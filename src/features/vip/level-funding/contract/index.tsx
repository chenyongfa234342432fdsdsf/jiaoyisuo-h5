...
function Contract(props) {
  ...
  useEffect(() => {
    getV1MemberVipBaseFeeListApiRequest({
      feeType: vipLevelFundingType.contract,
    }).then((res: any) => {
  ...
  useEffect(() => {
    ...
    if (result?.length) {
      setTradeFee({
        markerFeeRate: result[0]?.markerFeeRate,
        takerFeeRate: result[0]?.takerFeeRate,
      })
    }
  }, [searchKey])
  ...
  return (
    <div className={styles.scoped}>
      ...
      <div className="w-full px-4 box-border">
        <div className="common-14-title mt-4">{t`features_vip_level_fundting_contract_index_svswx_p6jf`}</div>
        <div className="text-brand_color common-12-content mt-2">{t`features_vip_level_fundting_contract_index_mvmspyodjm`}</div>
                ...
                {feeList?.map((item, index) => {
                  return (
                    <tr key={index}>
                      ...
                      >
                        {tradeFee?.markerFeeRate
                          ? `${(Number(100 - Number(item.makerFee)) * Number(tradeFee?.markerFeeRate)).toFixed(2)}%/${(
