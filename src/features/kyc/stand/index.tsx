...
export default function KycPersonStand() {
  ...
  // 选中图片
  const activeIcon = `${oss_svg_image_domain_address}login_verify_selected.png`
  const inactiveIcon = `${oss_svg_image_domain_address}login_verify_unselected_black.png`
        ...
        >
          <Form.Item name="country" label={t`features_user_register_residence_index_5101093`} className="area">
            <Cell
              ...
              icon={
                isEnble && (
                  <LazyImage src={`${oss_area_code_image_domain_address}${area?.remark}.png`} width={16} height={16} />
                )
              }
              onClick={() => setSelectArea(true)}
            />
            <Icon name="regsiter_icon_drop_white" className="absolute right-4" />
          </Form.Item>
          ...
          <div className="checkBox">
            <Checkbox
              ...
              shape="square"
              checked={isCheck}
              iconRender={createCheckboxIconRender('text-base')}
              onChange={val => {
                setIsCheck(val)
                if (!val) {
                  formData.certValidDateEnd = ''
