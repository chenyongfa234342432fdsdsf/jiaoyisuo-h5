...
import { Form, Button, Image, Toast, Uploader, UploaderValueItem, Flex, Dialog } from '@nbit/vant'
...
export default function KycUpLoad() {
  ...
  const handleUploader2 = async (list: Array<UploaderValueItem>) => {
    if (list.length > 0) {
      ...
      const res1 = await postUpload({ image: url })
      if (res1.isOk) {
        Toast.info(t`features_kyc_up_load_index_5101117`)
  ...
  function bgImg2(val, type) {
    if (val === 1) {
      if (type === 1) {
        return `${oss_svg_image_domain_address}icon_passport_standard.png`
      } else if (type === 2) {
        return `${oss_svg_image_domain_address}icon_passport_border_missing.png`
      } else if (type === 3) {
        return `${oss_svg_image_domain_address}icon_passport_blur.png`
      } else if (type === 4) {
        return `${oss_svg_image_domain_address}icon_passport_flash.png`
      } else {
        return ''
      }
    } else if (val === 2) {
      if (type === 1) {
        return `${oss_svg_image_domain_address}icon_driver_license_standard.png`
      } else if (type === 2) {
        return `${oss_svg_image_domain_address}icon_driver_license_border_missing.png`
      } else if (type === 3) {
        return `${oss_svg_image_domain_address}icon_driver_license_photo_blur.png`
      } else if (type === 4) {
        return `${oss_svg_image_domain_address}icon_driver_license_flash_strong.png`
      } else {
        return ''
      }
    } else if (val === 3) {
      if (type === 1) {
        return `${oss_svg_image_domain_address}icon_id_card_standard.png`
      } else if (type === 2) {
        return `${oss_svg_image_domain_address}icon_id_card_border_missing.png`
      } else if (type === 3) {
        return `${oss_svg_image_domain_address}icon_id_card_photo_blur.png`
      } else if (type === 4) {
        return `${oss_svg_image_domain_address}icon_id_card_flash_strong.png`
      } else {
        return ''
      }
    } else {
    ...
    <div className={`kyc-stand ${styles.scoped}`}>
      ...
      <div className="kyc-stand-wrap">
        ...
        <Form layout="vertical" form={form} onFinish={onFinish}>
          ...
          >
            <Uploader onChange={handleUploader} previewImage={false} accept=".jpg, .jpeg, .png">
              <div className="update-wrap" style={{ backgroundImage: `url("${''}")` }}>
                {!uploaderImage && <Icon className="icon_bg" name={bgImg(data.certType, 1)} />}
          ...
          >
            <Uploader onChange={handleUploader2} previewImage={false} accept=".jpg, .jpeg, .png">
              <div className="update-wrap" style={{ backgroundImage: `url("${''}")` }}>
                {!uploaderImage2 && <Icon className="icon_bg" name={bgImg(data.certType, 2)} />}
        ...
        <div className="text-xs text-text_color_03 mt-6">
          <Icon name="icon_certification_tips" className="mr-1 text-lg" />
