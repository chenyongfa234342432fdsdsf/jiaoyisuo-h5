...
import { Badge, SwipeCell, Button, Dialog } from '@nbit/vant'
import { getAuthModuleRoutes, AuthModuleEnum, getAuthMarketsModuleStatus } from '@/helper/modal-dynamic'
...
const InmailModal = ({ id, visible, onChangeCancel, onChangeConfirm }: InmailModalType) => {
  return (
    <Dialog
      ...
      className={styles['inmail-deleted-modal']}
    >
...
const Inmail = () => {
  ...
  const getInmailRouter = v => {
    link(`/inmail/message-window/${v?.id}?name=${v?.name}&code=${v?.codeName}`)
  }
  ...
  const getInmailTypeList = async () => {
    ...
    /** 分为头部三个和其他四个消息通知类型* */
    res.data.forEach(v => {
      const data = tagBox.find(item => item.id === v.codeName)
      if (data) {
        let list = { ...v, icon: data.icon, name: data.name }
        headerData.push(list)
      } else {
        const bodyList = menuList.find(params => {
          return params.id === v.codeName
        })
        let menuData = {
          ...v,
          icon: bodyList?.icon,
        ...
        menuBodyList.includes(v.codeName) && bodyData.push(menuData)
      }
    })
  ...
  const onChangeSwipeCell = (positon, data) => {
    positon === 'cell' && link(`/inmail/message-window/${data?.id}?name=${data?.name}&code=${data?.codeName}`)
  }
              ...
              return (
                <div className="grid-bottom-flex" key={v?.id} onClick={() => getInmailRouter(v)}>
                  <Badge content={v?.unReadNum} max={99} showZero={false}>
                    <LazyImage
                      src={`${oss_svg_image_domain_address}${v?.icon as string}`}
                      className="bottom-flex-icon"
                      imageType={Type.png}
                    />
                  </Badge>
                ...
                <div className="inmail-swipecell-box">
                  <div className="inmail-swipecell-content">
                    <Badge content={item?.unReadNum} max={99} showZero={false}>
                      <Icon name={item?.collapseIcon as string} hasTheme className="swipecell-box-icon" />
                    </Badge>
