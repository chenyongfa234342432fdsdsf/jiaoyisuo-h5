...
import { Input, Cell, Switch, Button, Dialog, Toast } from '@nbit/vant'
...
const ReminderPage = () => {
    ...
    Dialog.alert({
      message: (
        <div className="modal-wrap-content">
          ...
          <div className="modal-wrap-text">{t`features_inmail_reminder_page_index_73cdhng2gp`}</div>
          <div className="modal-wrap-text">{t`features_inmail_reminder_page_index_vmphixdnsr`}</div>
        </div>
      ),
      confirmButtonColor: '#101014',
      confirmButtonText: t`user.field.reuse_17`,
      className: styles['regsiter-modal-wrap'],
    })
  }

  useMount(() => {
    const params = type === KLineChartType.Quote ? marketState.currentCoin : contractMarketState.currentCoin
    getTradePairNotice()
  ...
  return (
    <div className={styles.scoped}>
      <NavBar
        title={`${reminderParams.baseSymbolName}${type === KLineChartType.Quote ? '/' : ''}${
      ...
      <div className="reminder-page-menu">
        ...
        <Button type="primary" onClick={onCreateReminder} className="page-menu-button" disabled={!priceValue}>
          <span
            className={`${!priceValue ? 'text-text_color_04' : 'button_text_02'}`}
