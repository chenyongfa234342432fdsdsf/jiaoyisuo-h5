import { MutableRefObject, ReactNode, useRef, useState, memo, useMemo } from 'react'
...
type ParamsRefType = Record<'endTime' | 'optionId' | 'period' | 'sideInd' | 'startTime' | 'timeRange', string> | null
...
>({
  ...
  const paramsRef = useRef<ParamsRefType>(null)

  const { settlementList, clearSettlementList } = useOptionPositionStore()
  const {
    list,
    loadMore,
    loading,
  ...
  useUpdateEffect(() => {
    if (!lodash.isEqual(paramsRef.current, params) && !loading) {
      clearSettlementList()
  ...
  const settlementMemoList = useMemo(() => {
    const uniqueSettleList = [...settlementList, ...list]
    const settleList = lodash.uniqBy(uniqueSettleList, 'id') as YapiGetV1OptionOrdersHistoryListData[]
    return settleList
  }, [settlementList?.length, list])

  const displayList = propList || (settlementMemoList?.length > 0 ? settlementMemoList : [])
  ...
  return (
    <div className={styles['order-list-layout-wrapper']} onTouchStart={onTouch}>
      <>
        ...
        <CommonList
          refreshing
          emptyClassName={styles['empty-img']}
      ...
      <FullScreenLoading isShow={loading} className={styles['order-loading']} />
    </div>
  )
}
