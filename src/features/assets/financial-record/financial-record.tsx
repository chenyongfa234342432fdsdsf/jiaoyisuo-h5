...
export function FinancialRecordLayout() {
  ...
  // tabs 展示数据
  const tabsTypeData = isFusionMode
    ? [main, contract, fee, other]
    : getAuthModuleRoutes({
        main,
        [AuthModuleEnum.c2c]: c2c,
        [AuthModuleEnum.contract]: contract,
        [AuthModuleEnum.options]: derivative,
        fee,
        commission,
        other,
      })
  ...
  const onLoadList = async (isRefresh?: boolean) => {
    const { startDate, endDate, coinId, type, status, rebateType } = assetsRecord[activeTab] || {}
    const rebateTypeEnums = assetsEnums.financialRecordRebateTypeList.enums
    ...
    let params: AssetsRecordsListReq = {
      ...
      pageSize: 20,
      logType: +logType || AssetsRouteEnum.coins,
      rebateType,
    }
    ...
    if (
      ...
      !rebateType ||
      rebateType.length === 0 ||
      rebateType.length === rebateTypeEnums.length
    )
  ...
  useEffect(() => {
    updateRecordModule({ activeTab: setFinancialRecordRoute(Number(logType)) || FinancialRecordRouteEnum.main })
  ...
  useUpdateEffect(() => {
    switch (activeTab) {
      ...
      case FinancialRecordRouteEnum.c2c:
        ...
        break
      case FinancialRecordRouteEnum.derivative:
        setTypeList(assetsEnums.financialRecordTypeDerivativeList.enums)
        break
      default:
  ...
  }, [assetsRecord, logType])

  useUpdateEffect(() => {
    activeTab === FinancialRecordRouteEnum.commission && requestWithLoading(onLoadList(true), 0)
  }, [assetsEnums.financialRecordRebateTypeList.enums])
        ...
        {tabsTypeData.map(({ title, name }) => (
          <Tabs.TabPane name={name} key={name} title={title}>
            <Record
              ...
              onShowScreenModal={() => setScreenVisible(true)}
            />
          </Tabs.TabPane>
        ))}
      </Tabs>
