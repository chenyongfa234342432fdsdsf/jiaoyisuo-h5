...
function FuturesOverview() {
  ...
  const accountNameRef = useRef<Record<'setOpenAccountActionSheet', () => boolean>>()

  const onSetAmount = (val: string) => {
    if (!val) return '--'
    return `${formatCurrency(val, futuresCurrencySettings.offset || 2)} ${!isFusionMode ? baseCoin : ''}`
  }
          ...
          <div>
            ...
            {+totalVoucherAmount > 0 &&
              t({
                ...
                values: { 0: onSetAmount(totalVoucherAmount) },
              })}
          </div>
        </>
      ),
  ...
  const operateInfo = [
    {
      title: t`features_assets_overview_tabs_futures_overview_index_jfu2m8ukxm`,
      onClick: () => accountNameRef.current?.setOpenAccountActionSheet(),
  ...
  return (
    <>
          ...
          {operateInfo.map((operate, index) => {
            ...
            return (
              <Button className={operate.className} key={index} onClick={operate.onClick}>
                <CommonDigital content={operate.title} />
              </Button>
            )
          })}
        </div>
      ...
      <CreateNewAccount ref={accountNameRef} />
    </>
  )
}
