...
import { getAssetsRechargePageRoutePath, getAssetsWithdrawPageRoutePath, getCapitalTransfer } from '@/helper/route'
...
function CoinOverview() {
  ...
  const onSetAmount = (amount = '', symbol = '', unit = 'code') => {
    ...
    return rateFilter({ amount, unit, symbol, isFormat: true, showUnit: !isFusionMode })
  }
  ...
  const withdraw = {
    ...
    onClick: () => link(getAssetsWithdrawPageRoutePath()),
  ...
  const operateInfo =
    isFusionMode || !getAuthModuleStatusByKey(AuthModuleEnum.c2c)
