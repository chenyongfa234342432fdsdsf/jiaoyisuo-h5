...
import { PullRefresh, Tabs } from '@nbit/vant'
...
import { FuturesListLayout } from './overview/list/futures/futures-layout'
...
function AssetsLayout() {
  ...
  const { fetchFuturesEnums, wsPerpetualGroupDetailSubscribe, wsPerpetualGroupDetailUnSubscribe } =
  ...
  const contract = {
    ...
    id: AssetsRouteEnum.contract,
    tabContent: <FuturesOverview />,
    content: <FuturesListLayout />,
  }
  const c2c = {
    title: 'C2C',
  ...
  const tabs = isFusionMode
    ? [overview, coins, contract]
    : getAuthModuleRoutes({ overview, coins, [AuthModuleEnum.contract]: contract, [AuthModuleEnum.c2c]: c2c })
  ...
  useEffect(() => {
    ...
    setDelay(activeTab === AssetsRouteEnum.overview || activeTab === AssetsRouteEnum.contract ? 5000 : undefined)
