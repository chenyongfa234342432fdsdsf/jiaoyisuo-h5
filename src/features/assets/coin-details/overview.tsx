...
import { getAssetsRechargePageRoutePath, getAssetsWithdrawPageRoutePath } from '@/helper/route'
    ...
    return (
      <Popup className={styles['lock-popup-wrapper']} visible={lockVisible} onClose={() => setLockVisible(false)}>
        ...
        <div className="lock-text">
          1. {t`features_assets_coin_details_overview_5101315`} - {t`features_assets_coin_details_overview_5101316`}
        </div>
        <div className="lock-text">2. {t`features_assets_coin_details_overview_5101097`}</div>
        <div className="lock-text">3. {t`features_assets_coin_details_overview_spp9keuxkj`}</div>
...
export function CoinActions({ coinId = '' }: { coinId: string }) {
  return (
    <div className={styles['details-buttons']}>
      <Button
        className="flex-1"
        onClick={() => link(getAssetsWithdrawPageRoutePath(coinId))}
      >{t`features_assets_coin_details_overview_510155`}</Button>
      <Button className="recharge-btn" type="primary" onClick={() => link(getAssetsRechargePageRoutePath(coinId))}>
