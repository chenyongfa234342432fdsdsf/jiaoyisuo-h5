...
export function CoinDetailsLayout() {
  ...
  const spotData = useWsSpotMarketTradePairRealTime({ apiData: isShowSpot ? tardeData?.spot : undefined })
  ...
  const onLoadDetails = async (coinId: string) => {
    const res = await requestWithLoading(postCoinDetail({ coinId }), 0)
    ...
    // 增加币种精度
    const newData = { ...data, coinPrecision: getCoinPrecision(data.symbol) }
    ...
    <div className={styles['page-wrapper']}>
      ...
      {!isFusionMode && (
        <div className="actions-wrapper">
          <CoinActions coinId={coinDetails.coinId} />
        </div>
      )}
    </div>
  )
