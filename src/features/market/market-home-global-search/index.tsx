...
export function MarketGlobalSearch() {
  ...
  const isFusionMode = useCommonStore().isFusionMode

  useEffect(() => {
    if (getAuthModuleStatusByKey(AuthModuleEnum.spot)) {
      store.setActiveModule(MarketLisModulesEnum.spotMarkets)
    } else if (getAuthModuleStatusByKey(AuthModuleEnum.contract)) {
      store.setActiveModule(MarketLisModulesEnum.futuresMarkets)
    }
  }, [])
    ...
    <div className={`${styles['market-search-common']} ${styles['market-search-global-wrapper']}`}>
      ...
      <Optional isRender={!!isJustFocused}>
        <div className="on-focus">
          {getAuthModuleStatusByKey(AuthModuleEnum.spot) && getAuthModuleStatusByKey(AuthModuleEnum.contract) && (
            <div className="selected-coin-history-panel">
          ...
          <div className="spot-hot-table">
            <div className="spot-hot-table-content">
              {store.activeModule === MarketLisModulesEnum.futuresMarkets || isFusionMode ? (
                <FuturesHotSearching from="kline" />
              ) : (
...
export const Optional = function ({ children, isRender }: { children: any; isRender: boolean }) {
