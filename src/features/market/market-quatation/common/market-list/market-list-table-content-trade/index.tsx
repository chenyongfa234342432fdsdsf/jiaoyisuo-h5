...
import useWsMarketTradePairList, {
  ...
  useWsMarketTernaryOptionTradePair,
...
// search ternary option trade default list
export function MarketListTradeTernaryOptionTableContent(props: { to?: 'kline' | 'trade' }) {
  ...
  const {
    ...
    refresh,
    apiStatus,
  } = useWsMarketTernaryOptionTradePair({ apiParams: { buyCoin: conceptId }, sorter: store.defaultSorter })
  ...
  return (
    <MarketSpotTradeSearchTableContent
      ...
      onRowClick={item => {
        store.setsearchHistory(item)
        store.resetSearchState()
        onTradePairClickTernaryOptionRedirect(item)
      }}
      showHeader={false}
      onPullRefresh={refresh}
      apiStatus={apiStatus}
...
export function MarketTradeSearchTernaryOptionTradeTable({ from }: { from: 'kline' | 'trade' }) {
  const upStore = useTernaryOptionStore()
  const store = useMarketListStore().ternaryOptionMarketsTradeModule
  const {
    resolvedData: data,
  ...
  } = useWsMarketTernaryOptionTradePair({ apiParams: { symbolName: store.searchInput } })
