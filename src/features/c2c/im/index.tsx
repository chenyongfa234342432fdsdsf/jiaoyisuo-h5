...
import { Image, Loading, Dialog, Input, Uploader, UploaderValueItem, Toast, Button } from '@nbit/vant'
...
import { useC2cImStore } from '@/store/c2c/im'
...
function CTcIm({ orderDetails }: { orderDetails: IC2cOrderDetail }) {
  ...
  const selfUid = c2cUserInfo.uid || userInfo.uid

  const { InitC2CIm } = useC2cImStore()
  ...
  const rowRenderer = ({ index, key, parent, style }) => {
    ...
    const dataName = data?.fromNick || data?.from || ''
    const buyerNickname =
  ...
  const getHistoryMsgsList = async () => {
    const defaultConfig = {
      scene: 'team',
      /** 群组 id */
      to: orderDetails?.tid || orderTid.current,
      limit: 100,
      asc: true,
    }

    /** 分页处理 100 条以内 不需要传 lastMsgId beginTime endTime */
    const currentConfiguration = isFirstGetHistory.current
      ? defaultConfig
      : imList.length > 1
      ? {
          ...defaultConfig,
          lastMsgId: lastMsg.current?.idServer,
          beginTime: 0,
          endTime: lastMsg.current?.time,
        }
      : defaultConfig

    const msgsList: Array<ImListType> = (await nimSDK.current?.msgLog.getHistoryMsgs(currentConfiguration)) || []
  ...
  /** 初始化 IM */
  const initIM = async () => {
    nimSDK.current = await InitC2CIm()

    if (!nimSDK.current) return

    /** 监听收到消息 */
    nimSDK.current.on('msg', (res: ImListType) => {
      if (res.target === orderDetails?.tid) {
        const list: Array<ImListType> = [...getImList(), res]
  ...
  /** 上传并发送图片 */
  const handleUploadImage = (list: Array<UploaderValueItem>) => {
  ...
  const onScrollChatList = item => {
    ...
    if (item.scrollTop < 250 && item.scrollTop > 0) {
      /** 大于等于 100 条调历史数据接口 */
      if (historyListLength.current >= 100) {
