      ...
      <NavBar
        title={
          <div className="flex items-center">
            ...
            {oppositeInfo?.isMerchant && (
              <LazyImage className="ml-1 w-4" src={`${oss_svg_image_domain_address}user_verified.png`} />
            )}
          </div>
        }
        right={
          <div className={classNames('text-sm order-status-nav-right flex justify-end', statusColor)}>
            <DomAutoScaleWrapper dep={statusText} className="text-right">
              {statusText}
            </DomAutoScaleWrapper>
          </div>
        }
