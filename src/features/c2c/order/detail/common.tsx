...
import { Button, Collapse, Toast } from '@nbit/vant'
      ...
      >
        <div onClick={toUserCenter} className="flex items-center justify-between">
          ...
          >
            ...
            {isMerchant && <LazyImage src={`${oss_svg_image_domain_address}user_verified.png`} className="w-4" />}
          </div>
          {withChat && (
            <div onClick={toIm} className="chat-wrapper">
          ...
          >
            ...
            <div className="flex w-0 flex-1 ml-2 text-text_color_01 justify-end text-right">
              {!item.lineClamp ? (
                ...
                <Tooltip className={valueClassName} content={item.copyValue || item.value}>
              ...
              {item.withCopy && (
                <TextWithCopy
                  onlyIcon
                  copyValue={item.copyValue || (item.value as any)}
                  text={item.copyValue || (item.value as string)}
                />
              )}
            </div>
          </div>
...
export function ActionsWrapper({ children, flex = true }: { children: ReactNode; flex?: boolean }) {
  return (
    <div
      className={classNames(styles['actions-wrapper'], 'whitespace-nowrap', {
        flex,
      })}
    >
      {children}
