...
import DomAutoScaleWrapper, { AutoScaleButton } from '@/components/dom-auto-scale-wrapper'
  ...
  return (
    <>
      ...
      <div className="flex flex-1">
        {showCancel && (
          <AutoScaleButton block onClick={cancel} className="gray-button mr-4">
            {t`features_c2c_order_cancel_index_oifgxckqnhvh7juckq092`}
          </AutoScaleButton>
        )}
        <AutoScaleButton onClick={run} loading={loading} disabled={sended} type="primary" block>
          {sended
...
export function ConfirmReceivedModal({
  ...
  return (
    <div className="px-4 text-sm">
      ...
      <div className="flex px-4 py-2 rounded bg-brand_color_special_02 mb-6">
        <Icon className="mr-2 text-xs translate-y-1" name="msg" />
        <div className="text-brand_color text-xs">{t`features_c2c_order_detail_paid_eecuoa33kzuyrivjramok`}</div>
      </div>
      <div className="mb-4">
