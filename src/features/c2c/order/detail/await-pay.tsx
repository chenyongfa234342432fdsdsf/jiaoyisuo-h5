...
import { Button, Dialog, Steps } from '@nbit/vant'
  ...
  return (
    <div>
      ...
      <ActionsWrapper>
        {!isNext && isBuy && (
          <>
            <AutoScaleButton onClick={cancel} block className="gray-button mr-4">
              {t`features_c2c_order_cancel_index_oifgxckqnhvh7juckq092`}
            </AutoScaleButton>
            <AutoScaleButton onClick={() => setIsNext(true)} type="primary" block>
              {t`user.field.reuse_23`}
            </AutoScaleButton>
          </>
        )}
        {isNext && isBuy && (
          <AutoScaleButton onClick={confirmTransferInner} type="primary" block>
            {t`features_c2c_order_detail_await_pay_i4g2nsupycslmrli5neer`}
          </AutoScaleButton>
        )}
        {!isBuy && (
          <AutoScaleButton disabled type="primary" block>
