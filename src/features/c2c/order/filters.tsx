...
import { Checkbox, Button, Toast, Selector } from '@nbit/vant'
...
export function getC2cOrderFiltersDefaultParams(tradeAreaIds?: any[]): IC2cQueryOrderListReq {
  return {
    ...
    beginDate: '' as any,
    endDate: '' as any,
    tradeTypes: [C2cOrderTradeTypeEnum.inside, C2cOrderTradeTypeEnum.outside],
  }
}
...
export function C2cOrderFilters() {
  ...
  const tradeTypeList = enumValuesToOptions(
    [C2cOrderTradeTypeEnum.inside, C2cOrderTradeTypeEnum.outside],
  ...
  return (
    <>
      <div onClick={() => onVisibleChange(true)}>
        <Icon className="text-xl" name="asset_record_filter" hasTheme />
      </div>
        ...
        <div className={styles['filters-modal-root']}>
          ...
          >
            {tradeTypeList.map(typeItem => {
              return (
                <Checkbox iconRender={checkboxIconRender} name={typeItem.value} key={typeItem.value} shape="square">
                  {typeItem.label}
                </Checkbox>
              )
            })}
