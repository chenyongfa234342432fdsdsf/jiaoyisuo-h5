...
function C2cTrade() {
  ...
  const [params, setParams] = useState<IQueryTradeAdListReq>({
    areaId: '',
    advertTypeCds: [],
    advertDirectCds: [urlParsed.search.tab || C2cTradeAdDirectionEnum.buy],
  })
  ...
  const tabs = [
    ...
    {
      label: t`features_c2c_trade_index_eztnyuoqdu`,
      value: C2cTradeAdDirectionEnum.handicap,
    },
  ]
  ...
  const renderListByTab = () => {
    switch (params.advertDirectCds?.[0]) {
      case C2cTradeAdDirectionEnum.handicap:
        return (
          <>
            <div className="rounded my-2 mx-4 w-[180px] bg-bg_sr_color">
              <CoinSelect
                inTrade
            ...
            <HandicapList
              currentCoin={params.currentCoin}
              currentTradeArea={params.currentTradeArea}
  ...
  return (
    <tradeContext.Provider value={contextValue}>
      <div className="flex flex-col h-screen">
        ...
        <C2cFooter />
        <div className="flex p-4 pb-2 overflow-x-auto">
          <TradeButtonRadios
