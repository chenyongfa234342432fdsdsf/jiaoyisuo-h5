...
import { Button, Dialog, Toast } from '@nbit/vant'
...
function TradeForm({
  ...
  const { run, loading } = useRequest(
    async () => {
      ...
      setAds(data.filter(item => currentTradeArea?.payments.some(name => name === item.paymentType)))
        ...
        <NavBar
          ...
          right={
            <Link href={getC2cOrdersPageRoutePath()}>
              <Icon name="asset_record" className="text-lg" hasTheme />
            </Link>
          }
        />
