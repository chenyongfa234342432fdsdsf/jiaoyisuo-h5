import { createC2cFastTradeBuyOrder, createC2cFastTradSellOrder, queryC2cTradeAdDetail } from '@/apis/c2c/trade'
...
function FormConfirm({ ads, unitIsAmount, isBuy, onVisibleChange, amount, count }: IFormConfirmProps) {
  ...
  const [selectedAd, setSelectedAd] = useState(
    ads.find(ad => {
      return ad.accounts.some(account => account.enabled)
    }) || ads[0]
  )
  ...
  return (
    ...
    >
      ...
      <div className="payment-list">
        {ads.map((adItem, index) => {
          ...
          return (
            ...
            >
              <div className="flex items-center">
                <PaymentItem payment={payment} className="text-sm" />
                {isBuy && isBest && (
      ...
      <div className="px-4 pb-10 flex">
        {(currentPaymentIsDisabled || currentPaymentIsEmpty) && !isBuy ? (
          <Button
            onClick={() => {
          ...
          <Button
            disabled={isBuy ? false : !currentPaymentAccount?.paymentId}
            onClick={run}
            loading={loading}
            block
...
export function C2cFastTradeConfirmModal({ visible, onVisibleChange, ...rest }: IFormConfirmProps) {
  ...
  return (
    <OnlyOnePopup visible={visible} position="bottom">
