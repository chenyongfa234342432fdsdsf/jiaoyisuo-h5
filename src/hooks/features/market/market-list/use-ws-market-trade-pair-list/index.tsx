...
import { TernaryTradePair, YapiGetV1OptionTradePairListApiRequestReal } from '@/typings/api/ternary-option/market'
import { getV1OptionTradePairListApiRequest } from '@/apis/market/ternary-option'
...
export function useWsMarketTernaryOptionTradePair({
  apiParams,
  sorter = quoteVolumneTableSorter,
}: {
  apiParams: YapiGetV1OptionTradePairListApiRequestReal
  sorter?: ColumnSort
}) {
  const [apiData, setApiData] = useSafeState<TernaryTradePair[]>([])
  const resolvedData = useWsTernaryOptionMarketTradePairFullAmount({ apiData })
  ...
  const apiRequest = (resolve, reject) => {
    getV1OptionTradePairListApiRequest(apiParams).then(res => {
      if (res.isOk) {
        const data = res?.data?.list || []
        // return resolve(sortByHelper(data, sorter))
        return resolve(data)
  ...
  const { refreshCallback: refresh, apiStatus } = useReqeustMarketHelper({
    ...
    setApiData,
    apiPreValidation,
    deps: [apiParams.buyCoin, apiParams.typeInd, apiParams.symbolName],
  })
