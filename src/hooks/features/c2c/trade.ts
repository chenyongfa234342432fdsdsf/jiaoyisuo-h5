...
export function useC2cToPublic() {
  ...
  useEffect(() => {
    if (!c2cMode.isPublic || !c2cMode.loaded) {
      return
    }
    const fn = async () => {
      const res = await loginWithFastPayApiRequest({})
      if (res.isOk && res.data) {
        location.replace(res.data.h5Url)
      } else {
        // 接口出错时返回首页
        link('/', {
    ...
    requestWithLoading(fn(), 0)
  }, [c2cMode.isPublic, c2cMode.loaded])

  return !c2cMode.isPublic && c2cMode.loaded
}
