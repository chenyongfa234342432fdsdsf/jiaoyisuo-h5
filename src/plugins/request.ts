...
import { gitCommitId, baseUrl, envIsDev, envIsServer, saasId } from '@/helper/env'
...
import { fastPayRequestInterceptor } from './request/interceptors/request/fast-pay-request'
...
export const getRequestBaseHeader = (headers: any) => {
  ...
  const { locale, accessKey, businessId } = baseCommonStore.getState()
  // 补齐 query 主题、语言等设置
  const lang = locale
...
const handleErrorCode = (
  ...
  if (isAuthError && !isAuthErrorShowMassge) {
    ...
    if (isFusionMode) {
      if (fusionModePopUpShow) return
      FusionModeLoginInvalidPopUp()
      fusionModePopUpShow = true
      return
    }
...
axiosRequest.interceptors.request.use(fastPayRequestInterceptor)
