...
import { OrderingUnitEnum } from './future/settings'
...
export function getEntrustTypeEnumName(type: EntrustTypeEnum) {
  return {
    ...
    [EntrustTypeEnum.stop]: t`features_trade_contract_contract_filters_index_5101498`,
  }[type]
}
/**
 * 保证金模式
...
export enum SpotStopLimitTypeEnum {
  /** 单向 */
  single = 'single',
  /** 双向 */
  double = 'double',
...
export enum SpotTriggerDirection {
  up = 'up',
  down = 'down',
}
...
export enum OptionCycle {
  Call = 'call', // 涨
  Put = 'put', // 跌
  OverCall = 'over_call', // 涨超
  OverPut = 'over_put', // 跌超
}
/** 合约交易委托账户类型 */

export enum FutureEntrustAccountTypeEnum {
  /** 长期账户 */
...
export const TradeUnitMap = {
  [OrderingUnitEnum.buy]: TradeUnitEnum.quote,
  [OrderingUnitEnum.sell]: TradeUnitEnum.indexBase,
}
export const TradeUnitMapReverse = {
  [TradeUnitEnum.quote]: OrderingUnitEnum.buy,
  [TradeUnitEnum.indexBase]: OrderingUnitEnum.sell,
}
/** 用户合约控制台交易状态 */
export enum UserFutureConsoleTradeStatusEnum {
  enabled = 1,
  disabled = 2,
}
/** 用户现货控制台交易状态 */
export enum UserSpotConsoleTradeStatusEnum {
  enabled = 1,
  onlyBuy = 2,
  onlySell = 3,
  disabled = 4,
