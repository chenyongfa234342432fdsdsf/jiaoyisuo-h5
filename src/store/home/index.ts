...
type HomeStore = MoreToolbarStore & {
  ...
  isDownloadPopupActive: boolean
  setIsDownloadPopupActive: (isActive: boolean) => void
}
...
function getStore(set: StoreApi<HomeStore>['setState'], get: StoreApi<HomeStore>['getState']): HomeStore {
  ...
  return {
    ...
    isDownloadPopupActive: true,
    setIsDownloadPopupActive: (isActive: boolean) =>
      set(
        produce((state: HomeStore) => {
          state.isDownloadPopupActive = isActive
        })
      ),
  }
}
