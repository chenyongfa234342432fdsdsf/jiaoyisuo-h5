...
import { YapiGetV1MemberVipBaseBusinessLimitData } from '@/typings/yapi/MemberVipBaseBusinessLimitV1GetApi'
import { getV1MemberVipBaseBusinessLimitApiRequest } from '@/apis/vip'
...
const vipLimitInfo = 'VIP_LIMIT_INFO'
...
function getBaseInfoStore(set) {
  return {
    ...
    async getFiatCurrencyData() {
      if (cacheUtils.get(fiatCurrencyData)) return
...
function getVipLimitInfoStore(set) {
  return {
    vipLimitInfo: cacheUtils.get(vipLimitInfo) || <YapiGetV1MemberVipBaseBusinessLimitData>{},
    async getVipLimitInfo() {
      const res = await getV1MemberVipBaseBusinessLimitApiRequest({})
      if (res.isOk && res.data) {
        set(
          produce((store: IStore) => {
            store.vipLimitInfo = res.data
            cacheUtils.set(vipLimitInfo, res.data)
          })
        )
      }
    },
...
function getStore(set) {
  return {
    ...
    avatar_url: '',
    setInitialAvatarUrl: (values: string) => {
      set((store: IStore) => {
        return produce(store, _store => {
          _store.avatar_url = values
        })
      })
    },
    ...getVipLimitInfoStore(set),
  }
}
