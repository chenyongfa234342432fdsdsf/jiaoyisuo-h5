...
import {
  ...
  FinancialRecordRebateTypeEnum,
  FinancialRecordRouteEnum,
  FinancialRecordStateEnum,
} from '@/constants/assets'
import { IAssetsRecordResp, IWithdrawFormData } from '@/typings/assets'
import {
  AssetsC2CListResp,
...
import {
  getAssetsFuturesAccountListFormCache,
  getAssetsDepositCoinHistoryCache,
  setAssetsDepositCoinHistoryCache,
  setAssetsFuturesAccountListFormCache,
} from '@/helper/cache'
import { IFuturesPositionListForm } from '@/features/assets/overview/list/futures/futures-list/list-layout'
import { ICoin } from '@/features/assets/common/coin-selection'
...
/** 提币表单默认数据 */
export const defaultWithdrawForm = {
  type: null as any, // 提币类型
  amount: '', // 提币数量
  coin: {} as ICoin, // 币种
  memo: '', // memo 地址
  addressVerify: true, // 是否通过提币地址校验
  network: {} as QuerySubCoinListSubCoinListResp, // 主网
  uid: '', // 收款人 uid
  amountInfo: {} as AssetsWithdrawCoinInfoResp, // 提币信息
  address: '', // 提币地址
}

const defaultAssetsFuturesAccountListFormCache = {
  sort: '',
  type: '',
} as IFuturesPositionListForm
...
function getStore(set, get) {
  return {
    ...
    assetsModule: {
      ...
      /** 合约账户列表筛选表单 */
      futuresAssetsAccountForm: getAssetsFuturesAccountListFormCache()
        ? { ...defaultAssetsFuturesAccountListFormCache, ...getAssetsFuturesAccountListFormCache() }
        : defaultAssetsFuturesAccountListFormCache,
      /** 合约资产列表 */
      ...
      updateFuturesAssetsAccountForm: (newFuturesAssetsAccountForm: IFuturesPositionListForm) =>
        set((store: IStore) => {
          return produce(store, _store => {
            const newAssetsModule = { ..._store.assetsModule, futuresAssetsAccountForm: newFuturesAssetsAccountForm }
            _store.assetsModule = newAssetsModule
            setAssetsFuturesAccountListFormCache(newFuturesAssetsAccountForm)
          })
        }),
      async fetchCoinRate() {
        const { isFusionMode } = baseCommonStore.getState()
        const res = isFusionMode ? await getHybridCoinRate({}) : await getV2CoinRate({})
      ...
      wsRateCallback: (data: Rate[]) => {
        ...
        store.assetsModule.updateAssetsModule({ coinRate: data[0] })
      },
      /** 币种汇率推送 */
      wsRateSubscribe: () => {
    ...
    withdrawModule: {
      ...
      /** 提现表单数据 */
      formData: defaultWithdrawForm as IWithdrawFormData,
      /** 提现结果数据 */
      withdrawResult: {
        isSuccess: false,
      ...
      updateCoinSearchHistory: (type: string, values: any) =>
        set((store: IStore) => {
          return produce(store, _store => {
            const newCoinSearchHistory = { ..._store.withdrawModule.coinSearchHistory, [type]: values }
            _store.withdrawModule.coinSearchHistory = newCoinSearchHistory
    ...
    assetsEnums: {
      ...
      /** 财务日志 - 返佣 - 返佣类型 */
      financialRecordRebateTypeList: {
        codeKey: AssetsDictionaryTypeEnum.recordRebateType,
        enums: [],
      } as IStoreEnum,
      /** 财务日志类型-c2c */
