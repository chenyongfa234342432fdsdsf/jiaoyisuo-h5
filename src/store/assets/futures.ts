...
import { AssetsDictionaryTypeEnum, AssetsRecordDateTypeEnum } from '@/constants/assets/common'
...
export const defaultFuturesPositionHistoryForm = {
  /** 合约币对 */
  symbol: '',
  tradeInfo: {} as YapiGetV1PerpetualTradePairListData,
  /** 平仓类型 */
  operationTypeCd: '',
  /** 时间类型 */
...
const defaultAssetsFuturesSettingCache = {
  /** 是否首次使用一键锁仓 */
  isFirstLock: true,
  /** 交易 - 是否隐藏其他合约 */
  hideOthers: false,
...
const defaultPositionStrategyCache = {
  /** 止盈止损触发类型 */
...
function getStore(set, get) {
  return {
    ...
    assetsFuturesSetting: getAssetsFuturesSettingCache()
      ? { ...defaultAssetsFuturesSettingCache, ...getAssetsFuturesSettingCache() }
      : defaultAssetsFuturesSettingCache,
    ...
    /** 合约组详情推送 */
    wsPerpetualGroupDetailSubscribe: (callback, throttleType = WSThrottleTypeEnum.cover) => {
      ws.subscribe({
    ...
    futuresPosition: {
      ...
      historyForm: defaultFuturesPositionHistoryForm as any,
