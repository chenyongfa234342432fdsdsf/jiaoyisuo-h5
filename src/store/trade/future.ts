...
import { getPersonalPreference, getSymbolTypeSettings, setSymbolTypeSettings } from '@/apis/trade'
...
const autoAddPreferenceDefault = {
  ...
  isAvg: PersonProtectTypeEnum.close,
  autoCloseIsolatedPosition: PriceProtectTypeEnum.close,
...
function getBaseStore(set, get) {
  const baseState = {
    settings: {
      ...
      async fetchTradeUnitSetting() {
        const state: IStore = get()
        const { data, isOk } = await getSymbolTypeSettings({})
        if (isOk) {
          const { symbolType } = data
          state.settings.updateTradeUnit(TradeUnitMap[symbolType])
        }
      },
      ...
      if (res.isOk) {
        set((store: IStore) => {
          return produce(store, _store => {
            ...
            const newData = {
              ...data,
              autoAddQuota: data?.autoAddQuota || autoAddPreferenceDefault.autoAddQuota,
              autoAddThreshold: data?.autoAddThreshold || autoAddPreferenceDefault.autoAddThreshold,
              autoCloseIsolatedPosition:
                data?.autoCloseIsolatedPosition || autoAddPreferenceDefault.autoCloseIsolatedPosition,
              hasOpenSpecializeVersion:
                data?.hasOpenSpecializeVersion || autoAddPreferenceDefault.hasOpenSpecializeVersion,
              isAutoAddExtra: data?.isAutoAddExtra || autoAddPreferenceDefault.isAutoAddExtra,
              isAutoAdd: data?.isAutoAdd || autoAddPreferenceDefault.isAutoAdd,
              isAvg: data?.isAvg || autoAddPreferenceDefault.isAvg,
              isSettingAutoMargin: data?.isSettingAutoMargin || autoAddPreferenceDefault.isSettingAutoMargin,
              marginSource: data?.marginSource || autoAddPreferenceDefault.marginSource,
              perpetualVersion: data?.perpetualVersion || autoAddPreferenceDefault.perpetualVersion,
              protect: data?.protect || autoAddPreferenceDefault.protect,
              retrieveWay: data?.retrieveWay || autoAddPreferenceDefault.retrieveWay,
            }
            _store.preferenceSettings = newData
            setCachePreference(newData)
          })
        })
      }
    },
    ...
    /** 教程中合约组是否可见 */
    tutorialFutureDetailVisible: false,
    setTutorialFutureDetailVisible: (value: boolean) => {
      set(
        produce((draft: IFutureTradeStore) => {
          draft.tutorialFutureDetailVisible = value
        })
      )
    },
  }
...
const baseFutureTradeStore = create(subscribeWithSelector(getStore))
...
baseFutureTradeStore.subscribe(
  state => state.settings.tradeUnit,
  value => {
    if (getIsLogin()) {
      setSymbolTypeSettings({ symbolType: TradeUnitMapReverse[value] })
    }
  }
)
