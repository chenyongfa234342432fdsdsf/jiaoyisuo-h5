...
const orderCOnfirmDefaultSettings = [
  ...
  [EntrustTypeEnum.stop, EntrustTypeEnum.limit, EntrustStopLimitType.stopProfitAndLoss],
...
export function getCommonSettings(set: any) {
  ...
  const settings = {
    ...
    updateEntrustPriceType(entrustPriceType: EntrustTypeEnum) {
      updateSettings(oldSettings => {
        oldSettings.entrustPriceType = entrustPriceType
      })
    },
  }
  return settings
