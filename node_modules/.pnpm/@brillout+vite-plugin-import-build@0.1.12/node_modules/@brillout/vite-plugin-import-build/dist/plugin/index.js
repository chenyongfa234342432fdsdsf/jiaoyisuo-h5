...
function importBuild(options) {
    ...
    return {
        ...
        configResolved(config_) {
            isSSR = (0, utils_1.viteIsSSR)(config_);
            if (!isSSR)
                return;
    ...
    function generateImporter(emitFile, rollupBundle) {
        ...
        const source = config.vitePluginDistImporter.libraries
            ...
            findBuildEntry: (entryName) => (0, findBuildEntry_1.findBuildEntry)(entryName, rollupBundle)
        }))
            .join('\n');
        emitFile({
    ...
    function setAutoImporter() {
        if (autoImporterIsDisabled())
            return;
        const distImporterFile = path_1.default.posix.join(getDistPathRelative(config), importBuildFileName_1.importBuildFileName);
        const { root } = config;
