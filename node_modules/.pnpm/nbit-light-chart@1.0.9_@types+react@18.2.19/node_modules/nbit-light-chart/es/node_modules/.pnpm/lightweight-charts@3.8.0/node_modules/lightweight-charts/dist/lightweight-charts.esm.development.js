...
import { bindToDevicePixelRatio } from '../../../../fancy-canvas@0.2.2/node_modules/fancy-canvas/coordinate-space.js';
/**
...
function setLineStyle(ctx, style) {
  ...
  var dashPatterns = (_a = {}, _a[0
  /* Solid */
  ] = [], _a[1
  /* Dotted */
  ] = [ctx.lineWidth, ctx.lineWidth], _a[2
  /* Dashed */
  ] = [2 * ctx.lineWidth, 2 * ctx.lineWidth], _a[3
  /* LargeDashed */
  ] = [6 * ctx.lineWidth, 6 * ctx.lineWidth], _a[4
  /* SparseDotted */
  ] = [ctx.lineWidth, 4 * ctx.lineWidth], _a);
...
function colorStringToRgba(colorString) {
  colorString = colorString.toLowerCase(); // eslint-disable-next-line no-restricted-syntax
...
function gradientColorAtPercent(topColor, bottomColor, percent) {
  ...
  var resultRgba = [normalizeRgbComponent(topR + percent * (bottomR - topR)), normalizeRgbComponent(topG + percent * (bottomG - topG)), normalizeRgbComponent(topB + percent * (bottomB - topB)), normalizeAlphaComponent(topA + percent * (bottomA - topA))];
...
function () {
  ...
  Delegate.prototype._internal_unsubscribeAll = function (linkedObject) {
    this._private__listeners = this._private__listeners.filter(function (listener) {
...
function merge(dst) {
  ...
  for (var _a = 0, sources_1 = sources; _a < sources_1.length; _a++) {
    var src = sources_1[_a]; // eslint-disable-next-line no-restricted-syntax
    ...
    for (var i in src) {
      ...
      if ('object' !== _typeof(src[i]) || dst[i] === undefined) {
        dst[i] = src[i];
      } else {
        merge(dst[i], src[i]);
      }
...
function clone(object) {
  ...
  var p;
  var v; // eslint-disable-next-line no-restricted-syntax
...
var PriceAxisRendererOptionsProvider =
/** @class */
function () {
...
function fillRectInnerBorder(ctx, x, y, width, height, borderWidth) {
  ...
  ctx.fillRect(x + borderWidth, y + height - borderWidth, width - borderWidth * 2, borderWidth); // vertical (left and right) edges
...
function () {
  ...
  PriceAxisView.prototype._internal_renderer = function (priceScale) {
    this._private__updateRendererDataIfNeeded(); // force update tickVisible state from price scale options
...
function () {
  ...
  TimeAxisViewRenderer.prototype._internal_draw = function (ctx, rendererOptions, pixelRatio) {
    ...
    var y1 = 0;
    var y2 = y1 + rendererOptions._internal_borderSize + rendererOptions._internal_paddingTop + rendererOptions._internal_fontSize + rendererOptions._internal_paddingBottom;
...
function (_super) {
  ...
  function Crosshair(model, options) {
    ...
    var valuePriceProvider = function valuePriceProvider(rawPriceProvider, rawCoordinateProvider) {
    ...
    var valueTimeProvider = function valueTimeProvider(rawIndexProvider, rawCoordinateProvider) {
      return function () {
        return {
    ...
    _this._private__currentPosPriceProvider = valuePriceProvider(function () {
      return _this._private__price;
    }, function () {
      return _this._private__y;
    });
    var currentPosTimeProvider = valueTimeProvider(function () {
  ...
  Crosshair.prototype._private__setIndexToLastSeriesBarIndex = function () {
    var lastIndexes = this._private__model._internal_serieses().map(function (s) {
...
function walkLine(ctx, points, lineType, visibleRange) {
  ...
  for (var i = visibleRange.from + 1; i < visibleRange.to; ++i) {
    var currItem = points[i]; //  x---x---x   or   x---x   o   or   start
...
function (_super) {
  ...
  PaneRendererAreaBase.prototype._internal__drawImpl = function (ctx) {
    ...
    setLineStyle(ctx, this._internal__data._internal_lineStyle); // walk lines with width=1 to have more accurate gradient's filling
...
function (_super) {
  ...
  */
  ...
  PaneRendererLine.prototype._internal__drawLine = function (ctx, data) {
    ...
    var items = data._internal_items,
        visibleRange = data._internal_visibleRange,
        lineType = data._internal_lineType,
        lineColor = data._internal_lineColor;
    ...
    for (var i = visibleRange.from + 1; i < visibleRange.to; ++i) {
      ...
      if (lineType !== 1
      /* WithSteps */
      && currentStrokeStyle !== prevStrokeStyle) {
        changeColor(currentStrokeStyle);
...
function (_super) {
  ...
  LinePaneViewBase.prototype._internal__fillRawPoints = function () {
    ...
    this._internal__items = this._internal__series._internal_bars()._internal_rows().map(function (row) {
      var value = row._internal_value[3
      /* Close */
      ];
...
function () {
  ...
  PaneRendererBars.prototype._internal_draw = function (ctx, pixelRatio, isHovered, hitTestData) {
    ...
    this._private__barWidth = this._private__calcBarWidth(pixelRatio); // grid and crosshair have line width = Math.floor(pixelRatio)
...
function (_super) {
  ...
  BarsPaneViewBase.prototype._internal__createDefaultItem = function (time, bar, colorer) {
    return {
      _internal_time: time,
      open: bar._internal_value[0
      /* Open */
      ],
      high: bar._internal_value[1
      /* High */
      ],
      low: bar._internal_value[2
      /* Low */
      ],
      close: bar._internal_value[3
      /* Close */
      ],
      _internal_x: NaN,
      _internal_openY: NaN,
  ...
  BarsPaneViewBase.prototype._internal__fillRawPoints = function () {
    ...
    this._internal__items = this._internal__series._internal_bars()._internal_rows().map(function (row) {
      return _this._internal__createRawItem(row._internal_index, row, colorer);
    });
  };
...
function (_super) {
  ...
  SeriesBarsPaneView.prototype._internal__createRawItem = function (time, bar, colorer) {
    return _assign(_assign({}, this._internal__createDefaultItem(time, bar, colorer)), {
      _internal_color: colorer._internal_barStyle(time)._internal_barColor
    });
  };
...
var PaneRendererCandlesticks =
/** @class */
function () {
  function PaneRendererCandlesticks() {
    this._private__data = null; // scaled with pixelRatio
  ...
  PaneRendererCandlesticks.prototype._private__calculateBorderWidth = function (pixelRatio) {
    ...
    * pixelRatio);
    ...
    if (this._private__barWidth <= res * 2) {
      ...
      return Math.max(Math.floor(pixelRatio), Math.floor(1
      /* BarBorderWidth */
      * pixelRatio));
    }
  ...
  PaneRendererCandlesticks.prototype._private__drawBorder = function (ctx, bars, visibleRange, barSpacing, pixelRatio) {
    ...
    for (var i = visibleRange.from; i < visibleRange.to; i++) {
      ...
      var left = Math.round(bar._internal_x * pixelRatio) - Math.floor(this._private__barWidth * 0.5); // this is important to calculate right before patching left
...
var SeriesCandlesticksPaneView =
/** @class */
function (_super) {
...
function () {
  ...
  PaneRendererHistogram.prototype._private__fillPrecalculatedCache = function (pixelRatio) {
    ...
    for (var i = this._private__data._internal_visibleRange.from; i < this._private__data._internal_visibleRange.to; i++) {
      var item = this._private__data._internal_items[i]; // force cast to avoid ensureDefined call
    ...
    for (var i = this._private__data._internal_visibleRange.from; i < this._private__data._internal_visibleRange.to; i++) {
      var current = this._private__precalculatedCache[i - this._private__data._internal_visibleRange.from]; // this could happen if barspacing < 1
...
function (_super) {
  ...
  SeriesHistogramPaneView.prototype._internal__convertToCoordinates = function (priceScale, timeScale, firstValue) {
    ...
    this._private__histogramData._internal_barSpacing = barSpacing; // need this to update cache
...
var SeriesLinePaneView =
/** @class */
function (_super) {
  __extends(SeriesLinePaneView, _super); // eslint-disable-next-line no-useless-constructor
...
function () {
  function TextWidthCache(size) {
    ...
    this._private__keysIndex = 0; // A trick to keep array PACKED_ELEMENTS
  ...
  TextWidthCache.prototype._internal_reset = function () {
    ...
    this._private__keys.fill(undefined); // We don't care where exactly the _keysIndex points,
...
var SeriesHorizontalLinePaneView =
/** @class */
function () {
...
var SeriesHorizontalBaseLinePaneView =
/** @class */
function (_super) {
  __extends(SeriesHorizontalBaseLinePaneView, _super); // eslint-disable-next-line no-useless-constructor
...
var SeriesLastPriceAnimationRenderer =
/** @class */
function () {
...
var animationStagesData = [{
  ...
  /* Stage2StartCircleRadius */
  ,
  _internal_endRadius: 14
  /* Stage2EndCircleRadius */
  ,
  _internal_startFillAlpha: 0
  /* Stage2StartFillAlpha */
  ,
  _internal_endFillAlpha: 0
  /* Stage2EndFillAlpha */
  ,
  _internal_startStrokeAlpha: 0.8
  /* Stage2StartStrokeAlpha */
  ,
  _internal_endStrokeAlpha: 0
  /* Stage2EndStrokeAlpha */
  ...
  /* Stage3StartCircleRadius */
  ,
  _internal_endRadius: 14
  /* Stage3EndCircleRadius */
  ,
  _internal_startFillAlpha: 0
  /* Stage3StartFillAlpha */
  ,
  _internal_endFillAlpha: 0
  /* Stage3EndFillAlpha */
  ,
  _internal_startStrokeAlpha: 0
  /* Stage3StartStrokeAlpha */
  ,
  _internal_endStrokeAlpha: 0
...
function animationData(durationSinceStart, lineColor) {
  var globalStage = durationSinceStart % 2600
  /* AnimationPeriod */
  / 2600
  /* AnimationPeriod */
  ;
...
function () {
  ...
  SeriesLastPriceAnimationPaneView.prototype._private__duration = function () {
    return this._internal_animationActive() ? performance.now() - this._private__startTime : 2600
    /* AnimationPeriod */
    - 1;
  };
...
function size(barSpacing, coeff) {
  var result = Math.min(Math.max(barSpacing, 12
  /* MinShapeSize */
  ), 30
  /* MaxShapeSize */
  ) * coeff;
    ...
    case 'inBar':
      {
        ...
        if (rendererItem._internal_text !== undefined) {
          rendererItem._internal_text._internal_y = rendererItem._internal_y + halfSize + shapeMargin + textHeight * (0.5 + 0.1
          /* TextMargin */
          );
        }
...
function () {
  ...
  SeriesMarkersPaneView.prototype._internal__makeValid = function () {
    ...
    if (this._private__dataInvalidated) {
      this._private__data._internal_items = seriesMarkers.map(function (marker) {
        return {
...
var SeriesPriceLinePaneView =
/** @class */
function (_super) {
  __extends(SeriesPriceLinePaneView, _super); // eslint-disable-next-line no-useless-constructor
  ...
  SeriesPriceLinePaneView.prototype._internal__updateImpl = function (height, width) {
    ...
    var lastValueData = this._internal__series._internal_lastValueData(seriesOptions.priceLineSource === 0
    /* LastBar */
    );
...
function (_super) {
  ...
  SeriesPriceAxisView.prototype._internal__updateRendererData = function (axisRendererData, paneRendererData, commonRendererData) {
    ...
    var showPriceAndPercentage = seriesOptions.seriesLastValueMode === 0
...
var CustomPriceLinePriceAxisView =
/** @class */
function (_super) {
...
function () {
  ...
  SeriesBarColorer.prototype._private__barStyle = function (barStyle, barIndex, precomputedBars) {
    var result = _assign({}, emptyResult);
    ...
    var isUp = ensure(currentBar._internal_value[0
    /* Open */
    ]) <= ensure(currentBar._internal_value[3
    /* Close */
    ]);
  ...
  SeriesBarColorer.prototype._private__areaStyle = function (areaStyle) {
    return _assign(_assign({}, emptyResult), {
  ...
  SeriesBarColorer.prototype._private__baselineStyle = function (baselineStyle, barIndex, precomputedBars) {
    var currentBar = ensureNotNull(this._private__findBar(barIndex, precomputedBars));
    var isAboveBaseline = currentBar._internal_value[3
    /* Close */
...
function () {
  ...
  PlotList.prototype._internal_contains = function (index) {
    return this._private__search(index, 0
    /* Exact */
    ) !== null;
  };
  ...
  PlotList.prototype._internal_search = function (index, searchMode) {
    ...
    return _assign(_assign({}, this._private__valueAt(pos)), {
  ...
  PlotList.prototype._private__minMaxOnRangeCachedImpl = function (start, end, plotIndex) {
    ...
    var result = null; // assume that bar indexes only increase
...
function (_super) {
  ...
  Series.prototype._internal_lastValueData = function (globalLast) {
    ...
    if (globalLast) {
      ...
      var endBar = this._private__data._internal_search(visibleBars._internal_right(), -1
      /* NearestLeft */
      );
    ...
    var price = bar._internal_value[3
    /* Close */
    ];
  ...
  Series.prototype._internal_applyOptions = function (options) {
    ...
    merge(this._private__options, options); // eslint-disable-next-line deprecation/deprecation
    ...
    if (options.priceFormat !== undefined) {
      this._private__recreateFormatter(); // updated formatter might affect rendering  and as a consequence of this the width of price axis might be changed
    ...
    this._internal_model()._internal_updateSource(this); // a series might affect crosshair by some options (like crosshair markers)
  ...
  Series.prototype._internal_dataAt = function (time) {
    ...
    if (this._private__seriesType === 'Bar' || this._private__seriesType === 'Candlestick') {
      return {
        open: prices._internal_value[0
        /* Open */
        ],
        high: prices._internal_value[1
        /* High */
        ],
        low: prices._internal_value[2
        /* Low */
        ],
        close: prices._internal_value[3
        /* Close */
        ]
      };
    } else {
      return prices._internal_value[3
      /* Close */
      ];
    }
  };
  ...
  Series.prototype._internal_markerDataAtIndex = function (index) {
    var getValue = (this._private__seriesType === 'Line' || this._private__seriesType === 'Area' || this._private__seriesType === 'Baseline') && this._private__options.crosshairMarkerVisible;
  ...
  Series.prototype._private__autoscaleInfoImpl = function (startTimePoint, endTimePoint) {
    ...
    var plots = this._private__seriesType === 'Line' || this._private__seriesType === 'Area' || this._private__seriesType === 'Baseline' || this._private__seriesType === 'Histogram' ? [3
    /* Close */
    ] : [2
    /* Low */
    , 1
  ...
  Series.prototype._private__recalculateMarkers = function () {
    ...
    this._private__indexedMarkers = this._private__markers.map(function (marker, index) {
      ...
      var searchMode = timePointIndex < firstDataIndex ? 1
      /* NearestRight */
      : -1
      /* NearestLeft */
      ;
...
function () {
  ...
  Magnet.prototype._internal_align = function (price, index, pane) {
    ...
    var candidates = serieses.reduce(function (acc, series) {
      ...
      return acc.concat([ps._internal_priceToCoordinate(bar._internal_value[3
      /* Close */
...
function () {
  ...
  PriceTickSpanCalculator.prototype._internal_tickSpan = function (high, low, maxTickSpan) {
    ...
    var index = 0;
    var c = this._private__integralDividers[0]; // eslint-disable-next-line no-constant-condition

    while (true) {
      // the second part is actual for small with very small values like 1e-10
      // greaterOrEqual fails for such values
      var resultTickSpanLargerMinMovement = greaterOrEqual(resultTickSpan, minMovement, 1e-14
      /* TickSpanEpsilon */
      ) && resultTickSpan > minMovement + 1e-14
      /* TickSpanEpsilon */
      ;
      var resultTickSpanLargerMaxTickSpan = greaterOrEqual(resultTickSpan, maxTickSpan * c, 1e-14
      /* TickSpanEpsilon */
      );
      var resultTickSpanLarger1 = greaterOrEqual(resultTickSpan, 1, 1e-14
      /* TickSpanEpsilon */
      );
    ...
    /* TickSpanEpsilon */
      ...
      while (greaterOrEqual(resultTickSpan, maxTickSpan * c, 1e-14
      /* TickSpanEpsilon */
      ) && resultTickSpan > minMovement + 1e-14
      /* TickSpanEpsilon */
      ) {
...
function () {
  ...
  PriceTickMarkBuilder.prototype._internal_rebuildTickMarks = function () {
    ...
    for (var logical = high - mod; logical > low; logical -= span) {
      var coord = this._private__logicalToCoordinateFunc(logical, firstValue, true); // check if there is place for it
...
function () {
  ...
  PriceScale.prototype._internal_setMode = function (newMode) {
    ...
    if (newMode._internal_mode !== undefined) {
      ...
      if (newMode._internal_mode === 2
      /* Percentage */
      || newMode._internal_mode === 3
      /* IndexedTo100 */
      ) {
    ...
    if (modeChanged && (oldMode._internal_mode === 2
    /* Percentage */
    || this._internal_isPercentage())) {
      this._internal_updateFormatter();
    }

    if (modeChanged && (oldMode._internal_mode === 3
    /* IndexedTo100 */
  ...
  PriceScale.prototype._internal_marks = function () {
    var firstValueIsNull = this._internal_firstValue() === null; // do not recalculate marks if firstValueIsNull is true because in this case we'll always get empty result
  ...
  */
  ...
  PriceScale.prototype._private__logicalToCoordinate = function (logical, baseValue) {
    ...
    var invCoordinate = this._private__bottomMarginPx() + (this._internal_internalHeight() - 1) * (logical - range._internal_minValue()) / range._internal_length();
  ...
  PriceScale.prototype._private__coordinateToLogical = function (coordinate, baseValue) {
    ...
    var range = ensureNotNull(this._internal_priceRange());
    var logical = range._internal_minValue() + range._internal_length() * ((invCoordinate - this._private__bottomMarginPx()) / (this._internal_internalHeight() - 1));
  ...
  PriceScale.prototype._private__recalculatePriceRangeImpl = function () {
    ...
    if (priceRange !== null) {
      ...
      if (priceRange._internal_minValue() === priceRange._internal_maxValue()) {
        ...
        var minMove = formatterSource === null || this._internal_isPercentage() || this._internal_isIndexedTo100() ? 1 : formatterSource._internal_minMove(); // if price range is degenerated to 1 point let's extend it by 10 min move values
  ...
  PriceScale.prototype._private__getCoordinateTransformer = function () {
    ...
    if (this._internal_isPercentage()) {
      ...
      return function (price) {
        return toLog(price, _this._private__logFormula);
      };
    }
...
function () {
  function Pane(timeScale, model) {
    ...
    this._private__leftPriceScale = this._private__createPriceScale("left"
    /* Left */
    , options.leftPriceScale);
    this._private__rightPriceScale = this._private__createPriceScale("right"
    /* Right */
  ...
  Pane.prototype._internal_setHeight = function (height) {
    ...
    this._private__rightPriceScale._internal_setHeight(height); // process overlays
  ...
  Pane.prototype._internal_removeDataSource = function (source) {
    ...
    var priceScale = source._internal_priceScale(); // if source has owner, it returns owner's price scale
  ...
  Pane.prototype._internal_scalePriceTo = function (priceScale, x) {
    priceScale._internal_scaleTo(x); // TODO: be more smart and update only affected views
...
var MMMM = function MMMM(date, locale) {
  return new Date(date.getUTCFullYear(), date.getUTCMonth(), 1).toLocaleString(locale, {
    month: 'long'
  });
};

var MMM = function MMM(date, locale) {
  return new Date(date.getUTCFullYear(), date.getUTCMonth(), 1).toLocaleString(locale, {
    month: 'short'
  });
};
...
function () {
  function DateTimeFormatter(params) {
    ...
    var formatterParams = _assign(_assign({}, defaultParams), params);
...
function defaultTickMarkFormatter(timePoint, tickMarkType, locale) {
  ...
  var date = timePoint._internal_businessDay === undefined ? new Date(timePoint._internal_timestamp * 1000) : new Date(Date.UTC(timePoint._internal_businessDay.year, timePoint._internal_businessDay.month - 1, timePoint._internal_businessDay.day)); // from given date we should use only as UTC date or timestamp
...
function () {
  ...
  FormattedLabelsCache.prototype._internal_format = function (value) {
    var cacheKey = value._internal_businessDay === undefined ? new Date(value._internal_timestamp * 1000).getTime() : new Date(Date.UTC(value._internal_businessDay.year, value._internal_businessDay.month - 1, value._internal_businessDay.day)).getTime();
...
function () {
  ...
  TickMarks.prototype._private__removeMarksSinceIndex = function (sinceIndex) {
    ...
    this._private__marksByWeight.forEach(function (marks, timeWeight) {
      if (sinceIndex <= marks[0]._internal_index) {
        ...
        marks.splice(lowerbound(marks, sinceIndex, function (tm) {
  ...
  TickMarks.prototype._private__buildMarksImpl = function (maxIndexesPerMark) {
    ...
    for (var _i = 0, _a = Array.from(this._private__marksByWeight.keys()).sort(function (a, b) {
      return b - a;
    }); _i < _a.length; _i++) {
      var weight = _a[_i];
      ...
      for (var i = 0; i < currentWeightLength; i++) {
        ...
        var currentIndex = mark._internal_index; // Determine indexes with which current index will be compared
...
function () {
  ...
  TimeScale.prototype._internal_timeToIndex = function (time, findNearest) {
    ...
    var index = lowerbound(this._private__points, time._internal_timestamp, function (a, b) {
  ...
  TimeScale.prototype._internal_setWidth = function (width) {
    ...
    if (this._private__options.fixLeftEdge) {
      ...
      if (visibleRange !== null) {
        var firstVisibleBar = visibleRange._internal_left(); // firstVisibleBar could be less than 0
        // since index is a center of bar


        if (firstVisibleBar <= 0) {
          var delta = this._private__width - width; // reduce  _rightOffset means move right
    ...
    this._private__width = width;
    this._private__visibleRangeInvalidated = true; // updating bar spacing should be first because right offset depends on it
  ...
  TimeScale.prototype._internal_setBarSpacing = function (newBarSpacing) {
    this._private__setBarSpacing(newBarSpacing); // do not allow scroll out of visible bars
  ...
  TimeScale.prototype._internal_marks = function () {
    ...
    var items = this._private__tickMarks._internal_build(spacing, maxLabelWidth); // according to indexPerLabel value this value means "earliest index which _might be_ used as the second label on time scale"
  ...
  */
  ...
  TimeScale.prototype._internal_zoom = function (zoomPoint, scale) {
    ...
    var newBarSpacing = barSpacing + scale * (barSpacing / 10); // zoom in/out bar spacing
  ...
  TimeScale.prototype._private__isAllScalingAndScrollingDisabled = function () {
    var _a = this._private__model._internal_options(),
        handleScroll = _a.handleScroll,
        handleScale = _a.handleScale;

    return !handleScroll.horzTouchDrag && !handleScroll.mouseWheel && !handleScroll.pressedMouseMove && !handleScroll.vertTouchDrag && !handleScale.axisDoubleClickReset && !handleScale.axisPressedMouseMove.time && !handleScale.mouseWheel && !handleScale.pinch;
  };
  ...
  TimeScale.prototype._private__coordinateToFloatIndex = function (x) {
    ...
    var index = baseIndex + this._private__rightOffset - deltaFromRight; // JavaScript uses very strange rounding
  ...
  TimeScale.prototype._private__setBarSpacing = function (newBarSpacing) {
    ...
    this._private__correctBarSpacing(); // this._barSpacing might be changed in _correctBarSpacing
  ...
  TimeScale.prototype._private__minRightOffset = function () {
    ...
    var barsEstimation = this._private__options.fixLeftEdge ? this._private__width / this._private__barSpacing : Math.min(2
    /* MinVisibleBarsCount */
  ...
  TimeScale.prototype._private__maxRightOffset = function () {
    return this._private__options.fixRightEdge ? 0 : this._private__width / this._private__barSpacing - Math.min(2
    /* MinVisibleBarsCount */
    , this._private__points.length);
  };
...
function weightToTickMarkType(weight, timeVisible, secondsVisible) {
  switch (weight) {
    ...
    /* Second */
    :
      return timeVisible ? secondsVisible ? 4
      /* TimeWithSeconds */
      : 3
      /* Time */
      : 2
...
function (_super) {
  ...
  return Watermark;
}(DataSource); /// <reference types="_build-time-constants" />
...
function () {
  ...
  ChartModel.prototype._internal_fullUpdate = function () {
    this._private__invalidate(new InvalidateMask(3
    /* Full */
    ));
  };

  ChartModel.prototype._internal_lightUpdate = function () {
    this._private__invalidate(new InvalidateMask(2
    /* Light */
    ));
  };
  ...
  ChartModel.prototype._internal_createPane = function (index) {
    ...
    var actualIndex = index === undefined ? this._private__panes.length - 1 : index; // we always do autoscaling on the creation
    // if autoscale option is true, it is ok, just recalculate by invalidation mask
    // if autoscale option is false, autoscale anyway on the first draw
    // also there is a scenario when autoscale is true in constructor and false later on applyOptions

    var mask = new InvalidateMask(3
    /* Full */
    );
  ...
  */
  ...
  ChartModel.prototype._internal_updateTimeScale = function (newBaseIndex, newPoints, firstChangedPointIndex) {
    ...
    var visibleBars = this._private__timeScale._internal_visibleStrictRange(); // if time scale cannot return current visible bars range (e.g. time scale has zero-width)
  ...
  ChartModel.prototype._internal_destroy = function () {
    ...
    this._private__panes.length = 0; // to avoid memleaks
  ...
  ChartModel.prototype._internal_moveSeriesToScale = function (series, targetScaleId) {
    ...
    pane._internal_removeDataSource(series); // check if targetScaleId exists
  ...
  ChartModel.prototype._internal_defaultVisiblePriceScaleId = function () {
    return this._private__options.rightPriceScale.visible ? "right"
    /* Right */
    : "left"
    /* Left */
    ;
  ...
  ChartModel.prototype._private__getBackgroundColor = function (side) {
    ...
    /* VerticalGradient */
      ...
      return side === 0
      /* Top */
      ? layoutOptions.background.topColor : layoutOptions.background.bottomColor;
    }
...
function getContext2D(canvas) {
  var ctx = ensureNotNull(canvas.getContext('2d')); // sometimes (very often) ctx getContext returns the same context every time
...
function createPreconfiguredCanvas(doc, size) {
  ...
  var pixelRatio = getCanvasDevicePixelRatio(canvas); // we should keep the layout size...

  canvas.style.width = "".concat(size._internal_w, "px");
  canvas.style.height = "".concat(size._internal_h, "px"); // ...but multiply coordinate space dimensions to device pixel ratio
...
function durationMSec(speed, dumpingCoeff) {
  ...
  return Math.log(1
  /* EpsilonDistance */
  * lnDumpingCoeff / -speed) / lnDumpingCoeff;
}

var KineticAnimation =
/** @class */
function () {
  ...
  KineticAnimation.prototype._internal_start = function (position, time) {
    ...
    var distance1 = distanceBetweenPoints(this._private__position1, this._private__position2); // We're calculating weighted average speed
    ...
    if (this._private__position3 !== null) {
      var speed2 = speedPxPerMSec(this._private__position2, this._private__position3, this._private__maxSpeed); // stop at this moment if direction of the segment is opposite
...
function () {
  function MouseEventHandler(target, handler, options) {
    ...
    this._private__lastTouchEventTimeStamp = 0; // for touchstart/touchmove/touchend events we handle only first touch
    ...
    this._private__onMobileSafariDoubleClick = function (dblClickEvent) {
      if (_this._private__firesTouchEvents(dblClickEvent)) {
        ...
        if (_this._private__tapTimeoutId && _this._private__tapCount > 1) {
          ...
          if (manhattanDistance < 30
          /* DoubleTapManhattanDistance */
          && !_this._private__cancelTap) {
        ...
        if (_this._private__clickTimeoutId && _this._private__clickCount > 1) {
          ...
          /* DoubleClickManhattanDistance */
          && !_this._private__cancelClick) {
  ...
  MouseEventHandler.prototype._private__touchMoveHandler = function (moveEvent) {
    ...
    var xOffset = moveInfo._internal_xOffset,
        yOffset = moveInfo._internal_yOffset,
        manhattanDistance = moveInfo._internal_manhattanDistance;

    if (!this._private__touchMoveExceededManhattanDistance && manhattanDistance < 5
    /* CancelTapManhattanDistance */
    ) {
      return;
    }
    ...
    if (!this._private__touchMoveExceededManhattanDistance) {
      ...
      var isHorzDrag = correctedXOffset > yOffset && !this._private__options._internal_treatHorzTouchDragAsPageScroll(); // if drag event happened then we should revert preventDefault state to original one
      ...
      this._private__touchMoveExceededManhattanDistance = true; // if manhattan distance is more that 5 - we should cancel tap event
    ...
    if (!this._private__preventTouchDragProcess) {
      ...
      this._private__processTouchEvent(compatEvent, this._private__handler._internal_touchMoveEvent); // we should prevent default in case of touch only
  ...
  MouseEventHandler.prototype._private__touchEndHandler = function (touchEndEvent) {
    ...
    if (this._private__tapTimeoutId && this._private__tapCount > 1) {
      ...
      if (!this._private__cancelTap) {
        this._private__processTouchEvent(compatEvent, this._private__handler._internal_tapEvent); // do not fire mouse events if tap handler was executed
    ...
    if (touchEndEvent.touches.length === 0) {
      if (this._private__longTapActive) {
        this._private__longTapActive = false; // prevent native click event
  ...
  MouseEventHandler.prototype._private__init = function () {
    ...
    this._private__target.addEventListener('mouseenter', this._private__mouseEnterHandler.bind(this)); // Do not show context menu when something went wrong
...

    {
      ...
      var outsideHandler_1 = function outsideHandler_1(event) {
    ...
    this._private__initPinch(); // Hey mobile Safari, what's up?
  ...
  MouseEventHandler.prototype._private__initPinch = function () {
    ...
    this._private__target.addEventListener('touchstart', function (event) {
  ...
  MouseEventHandler.prototype._private__mouseLeaveHandler = function (event) {
    ...
    this._private__processMouseEvent(compatEvent, this._private__handler._internal_mouseLeaveEvent); // accept all mouse leave events if it's not an iOS device
  ...
  MouseEventHandler.prototype._private__longTapHandler = function (event) {
    ...
    this._private__cancelTap = true; // long tap is active until touchend event with 0 touches occurred
  ...
  MouseEventHandler.prototype._private__makeCompatEvent = function (event, touch) {
    ...
    var box = this._private__target.getBoundingClientRect() || {
      left: 0,
      top: 0
    };
    return {
      ...
      _internal_preventDefault: function _internal_preventDefault() {
...
function () {
  ...
  LabelsImageCache.prototype._private__getLabelImage = function (ctx, text) {
    ...
    if (this._private__hash.has(text)) {
      ...
      var canvas = createPreconfiguredCanvas(document, new Size(width, height_1)); // Allocate new
...
function () {
  function PriceAxisWidget(pane, options, rendererOptionsProvider, side) {
    ...
    this._private__mouseEventHandler = new MouseEventHandler(this._private__topCanvasBinding.canvas, handler, {
      _internal_treatVertTouchDragAsPageScroll: function _internal_treatVertTouchDragAsPageScroll() {
        return false;
      },
      _internal_treatHorzTouchDragAsPageScroll: function _internal_treatHorzTouchDragAsPageScroll() {
        return true;
      }
    });
  }
  ...
  PriceAxisWidget.prototype._internal_optimalWidth = function () {
    ...
    var resultTickMarksMaxWidth = tickMarkMaxWidth || 34
    /* DefaultOptimalWidth */
    ;
    var res = Math.ceil(rendererOptions._internal_borderSize + rendererOptions._internal_tickLength + rendererOptions._internal_paddingInner + rendererOptions._internal_paddingOuter + resultTickMarksMaxWidth); // make it even
  ...
  PriceAxisWidget.prototype._internal_setSize = function (size) {
    ...
    if (this._private__size === null || !this._private__size._internal_equals(size)) {
      ...
      this._private__isSettingSize = false;
      this._private__cell.style.width = size._internal_w + 'px'; // need this for IE11
  ...
  PriceAxisWidget.prototype._internal_update = function () {
    var _a; // this call has side-effect - it regenerates marks on the price scale
  ...
  PriceAxisWidget.prototype._private__backLabels = function () {
    ...
    var addViewsForSources = function addViewsForSources(sources) {
  ...
  PriceAxisWidget.prototype._private__drawTickMarks = function (ctx, pixelRatio) {
    ...
    var tickMarkLeftX = this._private__isLeft ? Math.floor((this._private__size._internal_w - rendererOptions._internal_tickLength) * pixelRatio - rendererOptions._internal_borderSize * pixelRatio) : Math.floor(rendererOptions._internal_borderSize * pixelRatio);
    var textLeftX = this._private__isLeft ? Math.round(tickMarkLeftX - rendererOptions._internal_paddingInner * pixelRatio) : Math.round(tickMarkLeftX + rendererOptions._internal_tickLength * pixelRatio + rendererOptions._internal_paddingInner * pixelRatio);
  ...
  PriceAxisWidget.prototype._private__alignLabels = function () {
    ...
    var rendererOptions = this._internal_rendererOptions(); // if we are default price scale, append labels from no-scale
    ...
    var updateForSources = function updateForSources(sources) {
      sources.forEach(function (source) {
        var sourceViews = source._internal_priceAxisViews(paneState, priceScale); // never align selected sources
    ...
    views.forEach(function (view) {
      return view._internal_setFixedCoordinate(view._internal_coordinate());
    });
  ...
  PriceAxisWidget.prototype._private__setCursor = function (type) {
    this._private__cell.style.cursor = type === 1
    /* NsResize */
    ? 'ns-resize' : 'default';
  };
...
function () {
  function PaneWidget(chart, state) {
    ...
    this._private__mouseEventHandler = new MouseEventHandler(this._private__topCanvasBinding.canvas, this, {
      _internal_treatVertTouchDragAsPageScroll: function _internal_treatVertTouchDragAsPageScroll() {
        return _this._private__startTrackPoint === null && !_this._private__chart._internal_options().handleScroll.vertTouchDrag;
      },
      _internal_treatHorzTouchDragAsPageScroll: function _internal_treatHorzTouchDragAsPageScroll() {
        return _this._private__startTrackPoint === null && !_this._private__chart._internal_options().handleScroll.horzTouchDrag;
      }
    });
  }
  ...
  PaneWidget.prototype._private__drawSourceImpl = function (ctx, pixelRatio, paneViewsGetter, drawFn, source) {
    ...
    var objecId = hoveredSource !== null && isHovered && hoveredSource._internal_object !== undefined ? hoveredSource._internal_object._internal_hitTestData : undefined;
  ...
  PaneWidget.prototype._private__pressedMouseTouchMoveEvent = function (event) {
    ...
    if (this._private__startScrollingPos !== null && !this._private__isScrolling && (this._private__startScrollingPos.x !== event._internal_clientX || this._private__startScrollingPos.y !== event._internal_clientY)) {
      if (this._private__scrollXAnimation === null && (event._internal_isTouch && kineticScrollOptions.touch || !event._internal_isTouch && kineticScrollOptions.mouse)) {
        this._private__scrollXAnimation = new KineticAnimation(0.2
        /* MinScrollSpeed */
        , 7
        /* MaxScrollSpeed */
        , 0.997
...
function () {
  function PriceAxisStub(side, options, params, borderVisible, bottomColor) {
    ...
    this._private__canvasConfiguredHandler = function () {
      return _this._internal_paint(3
      /* Full */
      );
    };
...
function () {
  ...
  TimeAxisWidget.prototype._internal_optimalHeight = function () {
    ...
    rendererOptions._internal_borderSize + rendererOptions._internal_tickLength + rendererOptions._internal_fontSize + rendererOptions._internal_paddingTop + rendererOptions._internal_paddingBottom);
  };
  ...
  TimeAxisWidget.prototype._internal_paint = function (type) {
    ...
    /* Cursor */
      ...
      this._private__drawTickMarks(ctx, this._private__canvasBinding.pixelRatio); // atm we don't have sources to be drawn on time axis except crosshair which is rendered on top level canvas
  ...
  TimeAxisWidget.prototype._private__drawTickMarks = function (ctx, pixelRatio) {
    ...
    var maxWeight = tickMarks.reduce(markWithGreaterWeight, tickMarks[0])._internal_weight; // special case: it looks strange if 15:00 is bold but 14:00 is not
    ...
    var yText = rendererOptions._internal_borderSize + rendererOptions._internal_tickLength + rendererOptions._internal_paddingTop + rendererOptions._internal_fontSize - rendererOptions._internal_baselineOffset;
    ctx.textAlign = 'center';
    ctx.fillStyle = this._private__lineColor();
...
function () {
  function ChartWidget(container, options) {
    ...
    if (width === 0 || height === 0) {
      var containerRect = container.getBoundingClientRect(); // TODO: Fix it better
  ...
  ChartWidget.prototype._internal_destroy = function () {
    ...
    this._private__paneWidgets = []; // for (const paneSeparator of this._paneSeparators) {
  ...
  ChartWidget.prototype._internal_resize = function (width, height, forceRepaint) {
    ...
    if (forceRepaint) {
      this._private__drawImpl(new InvalidateMask(3
      /* Full */
      ));
    } else {
  ...
  ChartWidget.prototype._internal_paint = function (invalidateMask) {
    if (invalidateMask === undefined) {
      invalidateMask = new InvalidateMask(3
      /* Full */
      );
    }
  ...
  ChartWidget.prototype._internal_takeScreenshot = function () {
    ...
    drawScaled(ctx, pixelRatio, function () {
      ...
      var drawPriceAxises = function drawPriceAxises(position) {
        for (var paneIndex = 0; paneIndex < _this._private__paneWidgets.length; paneIndex++) {
          ...
          targetY += paneWidgetHeight; // if (paneIndex < this._paneWidgets.length - 1) {
      ...
      for (var paneIndex = 0; paneIndex < _this._private__paneWidgets.length; paneIndex++) {
        ...
        targetY += paneWidgetSize._internal_h; // if (paneIndex < this._paneWidgets.length - 1) {
        // 	const separator = this._paneSeparators[paneIndex];
        // 	const separatorSize = separator.getSize();
        // 	const separatorImage = separator.getImage();
        // 	ctx.drawImage(separatorImage, targetX, targetY, separatorSize.w, separatorSize.h);
      ...
      var drawStub = function drawStub(position) {
  ...
  ChartWidget.prototype._internal_getPriceAxisWidth = function (position) {
    ...
    var priceAxisWidget = position === 'left' ? this._private__paneWidgets[0]._internal_leftPriceAxisWidget() : this._private__paneWidgets[0]._internal_rightPriceAxisWidget();
  ...
  ChartWidget.prototype._private__adjustSizeImpl = function () {
    ...
    var paneWidth = Math.max(width - leftPriceAxisWidth - rightPriceAxisWidth, 0); // const separatorCount = this._paneSeparators.length;
    // const separatorHeight = SEPARATOR_HEIGHT;

    var separatorsHeight = 0; // separatorHeight * separatorCount;

    var timeAxisVisible = this._private__options.timeScale.visible;
    var timeAxisHeight = timeAxisVisible ? this._private__timeAxisWidget._internal_optimalHeight() : 0; // TODO: Fix it better
    // on Hi-DPI CSS size * Device Pixel Ratio should be integer to avoid smoothing
  ...
  ChartWidget.prototype._private__onMousewheel = function (event) {
    ...
    if ((deltaX === 0 || !this._private__options.handleScroll.mouseWheel) && (deltaY === 0 || !this._private__options.handleScale.mouseWheel)) {
      return;
    }
  ...
  ChartWidget.prototype._private__drawImpl = function (invalidateMask) {
    ...
    var invalidationType = invalidateMask._internal_fullInvalidation(); // actions for full invalidation ONLY (not shared with light)
  ...
  ChartWidget.prototype._private__syncGuiWithModel = function () {
    ...
    var actualPaneWidgetsCount = this._private__paneWidgets.length; // Remove (if needed) pane widgets and separators
    ...
    for (var i = targetPaneWidgetsCount; i < actualPaneWidgetsCount; i++) {
      ...
      paneWidget._internal_destroy(); // const paneSeparator = this._paneSeparators.pop();
    ...
    for (var i = actualPaneWidgetsCount; i < targetPaneWidgetsCount; i++) {
      ...
      this._private__paneWidgets.push(paneWidget); // create and insert separator
      // if (i > 1) {
  ...
  ChartWidget.prototype._private__getMouseEventParamsImpl = function (index, point) {
    ...
    var hoveredSeries = hoveredSource !== null && hoveredSource._internal_source instanceof Series ? hoveredSource._internal_source : undefined;
    var hoveredObject = hoveredSource !== null && hoveredSource._internal_object !== undefined ? hoveredSource._internal_object._internal_externalId : undefined;
    return {
...
function disableSelection(element) {
  element.style.userSelect = 'none'; // eslint-disable-next-line deprecation/deprecation
...
function () {
  function DataLayer() {
    ...
    this._private__seriesLastTimePoint = new Map(); // this is kind of "dest" values (in opposite to "source" ones) - we don't need to modify it manually, the only by calling _updateTimeScalePoints or updateSeriesData methods
  ...
  DataLayer.prototype._internal_setSeriesData = function (series, data) {
    ...
    var isTimeScaleAffected = false; // save previous series rows data before it's replaced inside this._setRowsToSeries
    ...
    if (prevSeriesRows !== undefined) {
      if (this._private__seriesRowsBySeries.size === 1) {
        ...
        isTimeScaleAffected = true; // perf optimization - if there is only 1 series, then we can just clear and fill everything from scratch
    ...
    if (isTimeScaleAffected) {
      ...
      newTimeScalePoints_1.sort(function (t1, t2) {
        return t1._internal_time._internal_timestamp - t2._internal_time._internal_timestamp;
      });
  ...
  DataLayer.prototype._internal_updateSeriesData = function (series, data) {
    ...
    var pointDataAtTime = this._private__pointDataByTimePoint.get(time._internal_timestamp); // if no point data found for the new data item
    ...
    var insertIndex = lowerbound(this._private__sortedTimePoints, newPoint._internal_time._internal_timestamp, function (a, b) {
      return a._internal_time._internal_timestamp < b;
  ...
  */
  ...
  DataLayer.prototype._private__replaceTimeScalePoints = function (newTimePoints) {
    var firstChangedPointIndex = -1; // search the first different point and "syncing" time weight by the way
  ...
  DataLayer.prototype._private__getUpdateResponse = function (updatedSeries, firstChangedPointIndex, info) {
    ...
    if (firstChangedPointIndex !== -1) {
      ...
      var seriesData = this._private__seriesRowsBySeries.get(updatedSeries); // if no seriesData found that means that we just removed the series
...
function checkPriceLineOptions(options) {
  // eslint-disable-next-line @typescript-eslint/tslint/config
  assert(typeof options.price === 'number', "the type of 'price' price line's property must be a number, got '".concat(_typeof(options.price), "'"));
}
...
function checkBarItem(type, barItem) {
  ...
  typeof barItem.open === 'number', "".concat(type, " series item data value of open must be a number, got=").concat(_typeof(barItem.open), ", value=").concat(barItem.open));
  assert( // eslint-disable-next-line @typescript-eslint/tslint/config
  typeof barItem.high === 'number', "".concat(type, " series item data value of high must be a number, got=").concat(_typeof(barItem.high), ", value=").concat(barItem.high));
  assert( // eslint-disable-next-line @typescript-eslint/tslint/config
  typeof barItem.low === 'number', "".concat(type, " series item data value of low must be a number, got=").concat(_typeof(barItem.low), ", value=").concat(barItem.low));
  assert( // eslint-disable-next-line @typescript-eslint/tslint/config
  typeof barItem.close === 'number', "".concat(type, " series item data value of close must be a number, got=").concat(_typeof(barItem.close), ", value=").concat(barItem.close));
}
...
function checkLineItem(type, lineItem) {
  ...
  assert( // eslint-disable-next-line @typescript-eslint/tslint/config
  typeof lineItem.value === 'number', "".concat(type, " series item data value must be a number, got=").concat(_typeof(lineItem.value), ", value=").concat(lineItem.value));
}
...
function () {
  ...
  SeriesApi.prototype.barsInLogicalRange = function (range) {
    ...
    var dataFirstBarInRange = bars._internal_search(correctedRange._internal_left(), 1
    /* NearestRight */
    );

    var dataLastBarInRange = bars._internal_search(correctedRange._internal_right(), -1
    /* NearestLeft */
    );

    var dataFirstIndex = ensureNotNull(bars._internal_firstIndex());
    var dataLastIndex = ensureNotNull(bars._internal_lastIndex()); // this means that we request data in the data gap
    ...
    var barsBefore = dataFirstBarInRange === null || dataFirstBarInRange._internal_index === dataFirstIndex ? range.from - dataFirstIndex : dataFirstBarInRange._internal_index - dataFirstIndex;
    var barsAfter = dataLastBarInRange === null || dataLastBarInRange._internal_index === dataLastIndex ? dataLastIndex - range.to : dataLastIndex - dataLastBarInRange._internal_index;
    var result = {
...
var chartOptionsDefaults = {
  ...
  overlayPriceScales: _assign({}, priceScaleOptionsDefaults),
  leftPriceScale: _assign(_assign({}, priceScaleOptionsDefaults), {
    visible: false
  }),
  rightPriceScale: _assign(_assign({}, priceScaleOptionsDefaults), {
    visible: true
  }),
...
function () {
  ...
  PriceScaleApi.prototype.width = function () {
    ...
    return this._private__chartWidget._internal_getPriceAxisWidth(this._private__priceScaleId === "left"
    /* Left */
    ? 'left' : 'right');
  };
...
function () {
  function ChartApi(container, options) {
    ...
    var internalOptions = options === undefined ? clone(chartOptionsDefaults) : merge(clone(chartOptionsDefaults), toInternalOptions(options));
  ...
  ChartApi.prototype.addBaselineSeries = function (options) {
    ...
    options = migrateOptions(options);
    patchPriceFormat(options.priceFormat); // to avoid assigning fields to defaults we have to clone them
...
export { ColorType, CrosshairMode, LastPriceAnimationMode as LasPriceAnimationMode, LastPriceAnimationMode, LineStyle, LineType, PriceLineSource, PriceScaleMode, TickMarkType, TrackingModeExitMode, createChart, isBusinessDay, isUTCTimestamp };
