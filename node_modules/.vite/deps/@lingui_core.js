...
// node_modules/.pnpm/@lingui+core@3.17.2/node_modules/@lingui/core/build/esm/compile.js
...
function compileMessage(message) {
  let mapText = arguments.length > 1 && arguments[1] !== void 0 ? arguments[1] : (v) => v;
  try {
...
// node_modules/.pnpm/@lingui+core@3.17.2/node_modules/@lingui/core/build/esm/index.js
...
function date(locales) {
  ...
  let memoize = arguments.length > 2 && arguments[2] !== void 0 ? arguments[2] : true;
  return (value) => {
    if (isString(value))
    ...
    if (memoize) {
      ...
      const formatter2 = new Intl.DateTimeFormat(locales, format);
      dateFormats.set(key, formatter2);
...
function number(locales) {
  ...
  return (value) => {
    if (memoize) {
      ...
      const formatter2 = new Intl.NumberFormat(locales, format);
      numberFormats.set(key, formatter2);
      return formatter2.format(value);
    }
...
var defaultFormats = function(locale, locales) {
  let localeData = arguments.length > 2 && arguments[2] !== void 0 ? arguments[2] : {
    plurals: void 0
  };
  let formats2 = arguments.length > 3 && arguments[3] !== void 0 ? arguments[3] : {};
  locales = locales || locale;
  const {
    plurals
  } = localeData;
  const style = (format) => isString(format) ? formats2[format] || {
    style: format
  } : format;
  const replaceOctothorpe = (value, message) => {
    return (ctx) => {
      ...
      const numberFormat = Object.keys(formats2).length ? style("number") : {};
...
function context(locale, locales, values, formats2, localeData) {
  const formatters = defaultFormats(locale, locales, localeData, formats2);
...
function interpolate(translation, locale, locales, localeData) {
  return function(values) {
    ...
    const ctx = context(locale, locales, values, formats2, localeData);
...
var I18n = class extends EventEmitter {
  ...
  _(id) {
    ...
    const messageMissing = !context2 && !this.messages[id];
    const contextualMessageMissing = context2 && !this.messages[context2][id];
    ...
    if (isString(translation))
      return translation;
    return interpolate(translation, this._locale, this._locales, this.localeData)(values, formats2);
  }
  date(value, format) {
